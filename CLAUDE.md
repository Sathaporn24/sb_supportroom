...
- **ใช้ shadcn semantic tokens เท่านั้น** (`bg-background`, `text-muted-foreground`, `bg-primary`,
  ...
  - ค่าที่จงใจต่างจาก Nova default (แก้ที่ `:root`/`.dark` ใน `globals.css` เท่านั้น):
    ...
    - `--tree-row` — พื้นหลังขาวล้วน (`oklch(1 0 0)` / `#ffffff`) ของแถวหมวดย่อยที่ยังไม่กางใน
      `CategoryTree.tsx` เท่านั้น (เพิ่ม 2026-08-28) — เท่ากับ `--card`/`--popover`, สว่างกว่า
      `--background` ของหน้าเล็กน้อยโดยตั้งใจ ให้แถวนี้ดูเป็นชั้นลอยบางๆ เหนือพื้นหลัง
      (ความสัมพันธ์เดียวกับที่ `--card` มีต่อ `--background`)
    - `--info`/`--info-foreground` — ปุ่มฟ้า (เช่น "+ เพิ่มบทเรียน" ใน `CategoryTree.tsx`)
      ที่ Figma แยกสีจาก primary orange
    - `--background` — `#fafafa` (`oklch(0.985 0 0)`) ปรับ 2026-08-28 ตามคำขอของผู้ใช้โดยตรง
      (ก่อนหน้านี้คือ `#fcfcfc` ปรับ 2026-08-25) — สีพื้นหลังทั้งหน้า ยังจงใจต่างจาก `--card`
      เล็กน้อยเพื่อให้การ์ดยังอ่านออกว่าลอยอยู่เหนือพื้นหลัง เหมือนเดิมก่อนหน้าการเปลี่ยนวันเดียวกันนี้
      ที่เคยลองตั้งเป็นขาวล้วนแล้วให้ `--tree-row` รับค่า `#fafafa` แทน — ภายในวันนั้นสลับกลับมาแบบนี้
    ...
    - `--icon-button-border`/`--icon-button-foreground` — เพิ่ม 2026-08-28 สำหรับปุ่มไอคอนเดี่ยว
      ขอบทั้งหมด (`variant="outline"` + `size="icon"/"icon-xs"/"icon-sm"/"icon-lg"` เช่น ปุ่มแก้/ลบ/
      ย้ายทุกจุดในแอป — แก้/ลบหมวดหมู่, แก้/ย้ายเอกสาร ฯลฯ) ตาม Figma icon-button component: ขอบ
      เข้มกว่า `--border` เล็กน้อย ไอคอนสีจางกว่า `--foreground` ปกติที่ variant="outline" ใช้ — ใส่
      เป็น `compoundVariants` ใน `ui/button.tsx` จุดเดียว **ไม่ใช่การแก้ทีละปุ่ม** ปุ่ม outline ที่มี
      ข้อความ (ไม่ใช่ icon-only) ไม่โดนกระทบ
    - ถ้าจะกลับไปใช้ Nova neutral ล้วน ต้องแก้ทั้ง 10 จุดนี้ ไม่ใช่แค่ `--primary` อีกต่อไป
