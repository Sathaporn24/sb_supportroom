...
## 13. Multi-engine execution (Claude Code + Codex)

This pipeline runs on two engines by design, not by accident: Claude Code directly, and Codex driven through `AGENTS.md`'s adapter chain (`AGENTS.md` → `CLAUDE.md` → this file → `.claude/agents/<role>.md`). Both read the same role definitions and the same module docs, so a role carries identical authority regardless of which engine is wearing it — a `qa-engineer` round Codex ran and a `qa-engineer` round a Claude subagent ran are the same kind of thing. Neither engine's output is "the real one" with the other as a stand-in or a rough draft.

**State lives in files, not in either engine's memory — that's what makes switching engines mid-task free.** §2's `status.md`, §4's amend discipline, and §12's "verify against real state" already make this true structurally: a module folder's docs are a complete, engine-agnostic record of what's decided and what's done. Whichever engine picks up next reads the same `plan.md`/`design.md`/`review.md` and continues from exactly where the other left off — the same way a second Claude Code session would resume a first one. Nothing about switching engines needs a handoff note beyond what §6 already requires.

**Don't re-run a role on the other engine just to double-check its own output.** If Codex completed a `qa-engineer` FULL round and ticked checkboxes accordingly, that round is accepted the same as if a Claude subagent had run it. Spinning up a Claude `qa-engineer` subagent afterward to redo the same phase from scratch is duplicated cost, not diligence — and it defeats the actual reason multi-engine execution exists, which is spreading work across engines (and their separate token/compute budgets), not doubling it on the same phase.

**Independent cross-verification is still worth asking for — deliberately, and scoped to what's actually in doubt.** For a specific finding that looks wrong, a Security-gated phase, or anything the user explicitly wants a second opinion on, ask the *other* engine to verify that finding — or re-run that phase — independently. This is legitimate and often valuable: a second, differently-calibrated pass catches things a single pass misses, the same way a human second reviewer does. The difference from the paragraph above is intent — a targeted second opinion asked for a reason, not a standing habit of re-verifying every result by default. State plainly when a round is a deliberate second opinion rather than the first look, and record the outcome (agree, or overturn-with-reasoning) the same way `qa-engineer`'s own recording discipline already requires for any judgment call. When a second pass disagrees with the first, treat it like any other file disagreement (§12): the better-evidenced read wins and the earlier one is corrected on the spot, not silently kept because it came first.
