...
- **Styling**: Tailwind CSS v4, theme defined in `src/app/globals.css` (`@theme inline` at top level, not inside `@layer base`) — use shadcn semantic tokens only (`bg-background`, `text-muted-foreground`, `bg-primary`, etc.), do not add CSS Modules, styled-components, or plain CSS files unless asked
- **Component system**: shadcn/ui on Base UI (`@base-ui/react`), style preset `base-nova`. `src/components/ui/` is shadcn primitives only (lowercase filenames, generated by the shadcn CLI) — never hand-write a primitive that shadcn already provides, and never put business logic there. Domain components live in `src/components/meeting/` and `.../admin/`; cross-domain composition in `.../shared/`. Check `frontend/.agents/skills/shadcn/SKILL.md` before adding a new component
- **Routing**: Next.js built-in file-based routing (`app/` directory) — do not add React Router
- **State management**: React built-in (`useState`/`useReducer`/Context) — no Zustand/Redux in this project. `src/tutor/` is a pure reducer; browser APIs belong in hooks (`src/hooks/`), not the reducer
- **Package manager**: npm — use `npm install`/`npm run`, never `yarn`/`pnpm`
- **Testing**: Vitest is already set up (`npm run test`) — keep the existing suite green; write tests when a task asks for them; `qa-engineer` runs it every round
