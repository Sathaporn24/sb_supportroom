...
- **Framework**: ASP.NET Core, .NET 10 — layered as `SupportRoom.Api` (controllers/hub/startup, kept thin) → `SupportRoom.Application` (services, DTOs, ViewModels, business orchestration) → `SupportRoom.Domain` (entities, contracts) → `SupportRoom.Providers.*` (external integrations). Real-time updates go through SignalR, not polling
- **Database**: PostgreSQL
- **ORM**: EF Core, in `SupportRoom.Providers.Data` — schema changes go through a new EF Core migration; never edit a migration that's already deployed
- **API style**: REST (resource-based controllers) + SignalR hub for real-time; no GraphQL/tRPC
- **Auth**: none implemented yet in this project (`/admin/*` is currently open — known gap, not a pattern to follow). Do not invent an auth scheme on your own; treat any auth work as a decision to confirm with the user first, same as any other stack change
- **Validation**: no standardized library in place yet — follow whatever pattern the module you're touching already uses (data annotations on ViewModels, or manual checks in the service); don't introduce a new validation library without asking
- **Package manager**: NuGet via `dotnet` CLI (`dotnet restore`/`dotnet build`), not npm
- **Testing**: xUnit, under `backend/tests/` (`SupportRoom.Api.IntegrationTests`, `SupportRoom.Application.Tests`, `SupportRoom.Providers.Tests`) — keep the existing suite green; write tests when a task asks for them; `qa-engineer` runs it every round. Some existing tests hit real providers rather than mocks — don't assume every failure is your code
