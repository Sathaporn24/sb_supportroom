...
5. Knowledge มี 2 scope: เฉพาะบทเรียน และ global ของบริษัทนั้น; ห้ามข้ามบริษัท
   > 🟡 **มีข้อเสนอขอรื้อข้อนี้ค้างอยู่ (บันทึก 2026-08-21) — ยังไม่ตัดสิน มติข้างบนยังมีผลบังคับเต็มที่**
   > ข้อเสนอ: ยุบ scope "เฉพาะบทเรียน" กับ "ทั้งบริษัท" ให้เหลือกองเดียว แล้วพึ่ง relevance scoring
   > ตอน retrieve แทน (**ไม่แตะกำแพงข้ามบริษัท**) · รายละเอียด ความเสี่ยง ทางสายกลาง และคำถาม
   > ที่ต้องตอบก่อน อยู่ที่ `_docs/module/knowledge-base/requirement.md` §Open Change Request (CR-1) ·
   > ต้องผ่าน `business-analyst` + `system-analyst` ก่อน จึงจะแก้ข้อความมติข้อ 5 นี้ได้
