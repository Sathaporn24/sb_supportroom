...
|---|---|---|
| audit-trail | **✅ 2026-08-27 (`system-analyst`): `design.md` เสร็จแล้ว · มติ 8 ข้อปิดครบ · Module A (ตาราง `AuditLog` + `SaveChanges` interceptor) + Module B (5 จุด raw SQL) ทั้งคู่ติด 🔒 Security gate · additive ล้วน ไม่มี breaking change** · **✅ 2026-08-28 (`system-analyst` amend · เอกสารล้วน ไม่มีโค้ด ไม่มี migration ไม่แตะ Data Model): OQ-A1 ปิดแล้ว = ทางเลือก ก `CompanyId` ของแถว log ที่ entity เป็น `AdminUser` เป็น `null` เสมอ (ระดับระบบ) ตามคำตอบเดิมของ OQ-2 — AU-11 ที่เป็น default อยู่แล้วถูกต้อง ไม่ต้องแก้สัญญาข้อใดเลย · trade-off ที่ยอมรับแล้วบันทึกที่ AR-10 (dashboard ในอนาคตของ `admin` ระดับบริษัทจะไม่เห็นว่าใครแก้/ปิดบัญชีพนักงานในบริษัทตัวเอง เห็นได้เฉพาะ `owner`) · **ไม่มีคำถามค้างในโมดูลนี้แล้ว** (OQ-A2/OQ-A3/OQ-A4 = เลื่อนโดยตั้งใจ ไม่ใช่รอคำตอบ)** · **🆕 2026-08-27 (`business-analyst`): โมดูลใหม่ · `requirement.md` รอบแรกเขียนเสร็จ (R1–R6) · OQ-1 ปิดแล้ววันเดียวกัน** — บันทึกประวัติการกระทำแบบดิบ ทุก entity ทุก create/update/delete **เฉพาะครั้งที่ `AdminUser` (owner/admin/cs) เป็นผู้ก่อ** · **⛔ ไม่รวมกิจกรรมที่ผู้เรียนก่อ** (`LearningSession`/`SessionQuestion` ฯลฯ อยู่นอกขอบเขต — เส้นแบ่งคือ "ใครลงมือ" ไม่ใช่ "ตารางไหน" ตาม R1.2) · ไม่เก็บ diff (ตอบไม่ได้ว่าค่าเปลี่ยนจากอะไรเป็นอะไร — ตั้งใจ ไม่ใช่ช่องโหว่ ดู C5) · เก็บถาวรไม่มี retention · แยกตามบริษัท · ไม่มี UI/dashboard ในรอบนี้ ("พร้อมทำ dashboard" = ข้อจำกัดเรื่องรูปร่างข้อมูล ไม่ใช่ใบอนุญาตสร้าง UI ตาม R5.1) · มี `design.md` แล้ว ยังไม่มี `plan.md`/โค้ด · **⛔ ห้าม apply migration จนกว่าเจ้าของโปรเจกต์จะให้ไฟเขียวแยกต่างหาก** (เจ้าของเป็น jr. dev ไม่มีสิทธิ์จัดการ DB · DB จริงเพิ่ง provision โดยทีม ops ภายนอก) — **ไฟเขียวนี้ไม่ผูกกับความคืบหน้าของเอกสาร** เอกสารเสร็จหรือ `design.md` เสร็จ ไม่นับเป็นไฟเขียว · OQ-2 (owner ที่ `CompanyId = null` ลง log บริษัทไหน) · OQ-3 (การเปลี่ยนแปลงที่ระบบก่อเอง `CreateBy = null`) · OQ-4 (login/logout/login ล้มเหลว อยู่ในขอบเขตไหม) · OQ-5 (`cs` เห็น log ไหม ตอนมี dashboard) — **✅ ทั้งสี่ข้อปิดครบแล้วใน `design.md` 2026-08-27** (OQ-2 nullable ไม่มี query filter · OQ-3 ไม่บันทึก action ของระบบ · OQ-4 ตัด login ออกจากรอบนี้ · OQ-5 `cs` ไม่เห็น log) | **✅ 2026-08-28 (`project-manager`): `plan.md` เสร็จแล้ว** — Phase 1 = Module A, Phase 2 = Module B (ขึ้นกับ Phase 1), ทั้งคู่ `🔒 Security gate`, งานทั้งหมด `[backend]` | `security` — ทั้ง Phase 1/Phase 2 implemented + verified ✅ (FULL) แล้ว รอ `security` audit ก่อน `devops` ship ได้ (🔒 Security gate ทั้งคู่ยังเปิดอยู่) |
| knowledge-base | **QA FULL (2026-08-20) — Phase 7 verified ✅ (FULL)**, module overall ✅ — 139/140 `plan.md` tasks ✅ ติ๊กแล้วทั้ง 7 phase (เหลือแค่ R-2 latency ที่บล็อกด้วย deployment ไม่ใช่ล้มเหลว), ไม่มีข้อค้าง ❌/⚠️ เหลือเลย. Phase 7 (Document scope assignment, Module G, 🔒 gate) 22/22 ✅ ยืนยันด้วยโค้ดจริง + unit test ใหม่ 14 ตัว (backend 204/204) + ทดสอบจริงกับแอปที่รันอยู่ (curl ยิง 6 กรณีปฏิเสธของ DS-3 ผ่าน JWT จริง, login+`GET /api/companies` จริง) · Phase 1's TX-5 partial ปิดแล้ว (ปิดพร้อม Phase 7 wiring call site) → Phase 1 ตอนนี้ 15/15 ✅ · **บั๊ก 2 (owner บริษัทเดียว login ไม่ได้)** แก้ใน `AdminSessionProvider.tsx` ยืนยันแล้วด้วย code trace + live data (`GET /api/companies` คืนบริษัทเดียวจริงสำหรับ `owner@local.test`) — ไม่มี browser tool ในเซสชันนี้ให้คลิกทดสอบเอง แต่ผู้ใช้ยืนยันด้วยตาตัวเองแล้ว · **บั๊ก 3 (Select ต้องเลือกสองรอบ)** แก้ใน `DocumentUploadList.tsx` + `KnowledgeQnAAnswerDialog.tsx` (ไฟล์ Phase 6 เดิม, regression check ผ่านครบไม่กระทบ flow เดิม) ยืนยันแล้วด้วย diff อ่านโค้ดตรง · Phase 3 ยังเหมือนเดิม (21/21 ✅ แต่รอบล่าสุดเป็น TARGETED ไม่ใช่ FULL ต้องมี FULL ก่อน deploy ได้) · **`security` ยังไม่เคยรันสักครั้งตลอดทั้ง 7 phase** — เป็นตัวบล็อกเดียวที่เหลือสำหรับทุก phase ที่ติด gate (2,3,4,6,7) ก่อนส่ง `devops` · รายละเอียดเต็มใน `review.md` (Phase 1–6 เดิมย้ายไป `review/phase-1-6.md` แล้ว, Phase 3 Round 1 เดิมอยู่ที่ `review/phase-3.md`) | `security` (เมื่อผู้ใช้เรียก), `devops` (Phase 1 พร้อม accept ได้ทันที ไม่ติด gate), **✅ 2026-08-25 (`system-analyst` amend เสร็จแล้ว): R7/R8 เข้าออกแบบครบ · DS-8/Q-C ยกเลิกตาม CR-1.j · เกิด contract ใหม่ KL-1..KL-18 + UC-1..UC-10 และ Module H (🔒 gate) + Module I · endpoint ใหม่ตัวเดียว `GET /api/knowledge-qna` · ⚠️ R7.6 ยังไม่เสร็จทั้งข้อ — หลังบ้าน+api-client ครบแต่ไม่มี call site ใน UI เลย** · **✅ 2026-08-25 (รอบสอง · `system-analyst` amend เฉพาะจุด): `Q-H1` ปิดแล้ว = ทาง B** — `DocumentResource.ContentHash` (`string?`) + index `(CompanyId, ContentHash)` เข้า Data Model (DM-3/DM-15) แล้ว · contract R7.5 เต็มที่ **KL-18..KL-24** · **migration ใหม่ 1 ใบ `MG-H1` = `AddDocumentContentHash` (additive ล้วน ไม่ backfill · generate ตอน implement เท่านั้น)** · `UploadDocumentDto.CheckDuplicate` default `false` (ไม่ breaking · ไม่กระทบ `handlePdfUpload` ตาม UC-5) · **ไม่มีคำถามค้างในโมดูลนี้แล้ว** · **✅ 2026-08-25 (`project-manager`): `plan.md` amended — เพิ่ม Phase 8 (Module H, 🔒 gate, รวม MG-H1/KL-18..KL-24 เฟสเดียวกัน) และ Phase 9 (Module I, ไม่ติด gate, ต้อง deploy หลัง Phase 8 เสมอตาม R-18) พร้อม task tag `[frontend]`/`[backend]` ครบ ไม่แตะ checkbox เดิมของ Phase 1–7** → **ผู้รับต่อ: `frontend-engineer`/`backend-engineer`** (Phase 8 ก่อน แล้ว Phase 9), `qa-engineer` verify เมื่อ implement เสร็จ · **✅ 2026-08-25 (รอบสาม · `system-analyst` amend reconcile Phase 8 · ไม่มีโค้ด ไม่แตะ checkbox ไม่แตะ `requirement.md`)**: engineer ทั้งสองฝั่ง implement Phase 8 แล้วตีกลับ 3 จุดที่ contract ไม่ตรงของจริง — reconcile ครบทั้งสาม **ไม่แตะ Data Model แม้ฟิลด์เดียว** · (1) KL-21 `createDate` → **`createdAt`** + 409 ขี่ใน `ApiErrorResponse.error.details` ไม่ใช่ body เปล่า (**เอกสารผิด โค้ดถูก**) · (2) **KL-25 ใหม่: หน้า `/admin/documents` ไม่มีทางสร้าง Q&A ใหม่ ทางสร้างอยู่ที่ `/admin/qna-queue` ที่เดียว** — `requirement.md` R7.2/R7.6/CR-1.h ไม่เคยขอ และตาม QQ-2/QQ-7 สร้างไม่ได้อยู่แล้ว (`SessionQuestionIds` ต้องมี ≥ 1) → `frontend-engineer` ทำถูก · ถ้อยคำ task ใน `plan.md` ที่คลาดเคลื่อนถูกแก้ **phasing ไม่เปลี่ยน ไม่ต้องเรียก `project-manager`** · (3) **รับการเทียบซ้ำแบบ in-memory เข้าเป็นข้อบังคับใน KL-19/KL-20/KL-23 พร้อมคำสั่ง "ห้ามแก้กลับเป็น `ILike`"** — เหตุผลหลักคือความถูกต้อง (`ILike` ตีความ `_`/`%` ในชื่อไฟล์เป็น wildcard = false positive) ไม่ใช่แค่ testability · KL-11 ยังเป็น `ILike` จริงตามเดิม · **🔴 เปิด `Q-H2` รอเจ้าของโปรเจกต์เคาะ (ไม่บล็อกงานอื่น แต่บล็อกการปิด KL-23)**: คำเตือน Q&A ซ้ำขึ้นหลังบันทึกสำเร็จ ทำให้ปุ่ม "บันทึกเพิ่มอยู่ดี"/"ไปแก้ใบเดิมแทน" ให้ผลเหมือนกันเป๊ะคือปิด dialog — เลือกระหว่าง (ก) แก้ถ้อยคำ + deep-link `?q=` หรือ (ข) เปลี่ยนเป็นด่านก่อนบันทึกแบบ KL-21 · **✅ 2026-08-25 (รอบสี่ · `system-analyst` amend เฉพาะจุด): `Q-H2` ปิดแล้ว = ทาง (ข) ด่านก่อนบันทึก** — KL-23 เขียนใหม่ (ตรวจก่อนเขียนทุกอย่าง → 409 `CONFLICT` + `DuplicateQnAResponse` ใน `error.details` · การตรวจ unconditional ไม่มีธง `CheckDuplicate` เพราะทางสร้างมี call site เดียว · ธงที่เพิ่มคือ `CreateKnowledgeQnADto.ConfirmDuplicate` default `false`) + **KL-26 ใหม่** (UI: "แก้ใบเดิมแทน" สลับเป็น `mode: "edit"` ในที่เดิม · ลบ prop `onEditExisting` · **ห้ามลิงก์ `/admin/documents?q=`** · การแก้ใบเดิมไม่ปิดคำถามในคิวและต้องเขียนบอก) + **O-13 ใหม่** (ผูกคำถามในคิวเข้ากับ Q&A เดิม = ความต้องการใหม่ ต้องกลับไป `business-analyst`) · **ไม่มีฟิลด์ใหม่ ไม่มี migration Data Model ไม่ถูกแตะ** · breaking เฉพาะ wire ของ `POST /api/knowledge-qna` (caller เดียว) · **ต้องรื้อโค้ด Phase 8 ที่เพิ่ง implement เสร็จ: `CreateAsync` · DTO · response shape · dialog ทาง create · test 3 ตัว** → **ผู้รับต่อ: `project-manager`** (เขียน task Phase 8 ใหม่ก่อน engineer หยิบ) · `qa-engineer` ยังห้ามปิด KL-23 เป็น ✅ · **🆕 2026-08-26 (`business-analyst` · CR-2): เพิ่ม R4.6 เฟสจัดการเนื้อหาก่อนสร้างบทเรียน PDF (R4.6.1–R4.6.10) — เลือกไฟล์ไม่ commit อะไร, กด "สร้าง" เปิดเฟสรีวิวสไลด์ภาพจริง+บทพูดทีละหน้า, "ยืนยันการสร้าง" เป็นจุด commit จุดเดียว, ไม่มี state ที่ server ก่อนยืนยัน · R8.3 ("ห้ามแตะ `handlePdfUpload`") ถูก supersede อย่างเป็นทางการ · CR-2.h (scope เอกสาร PDF ตอน commit) มอบให้ `system-analyst` ตัดสิน → ผู้รับต่อ: `system-analyst` amend `NR-1..NR-9` ให้ครอบเคสยังไม่มี `LessonId` · ⛔ ยังไม่ส่ง engineer** · **✅ 2026-08-26 (`system-analyst` amend รับ CR-2 เสร็จแล้ว · ไม่มีโค้ด ไม่แตะ checkbox ไม่แตะ `plan.md`/`requirement.md`): เกิด Module J (🔒 Security gate) · NR-1/NR-2/NR-3/NR-5/NR-6/NR-8/NR-9 amend เฉพาะจุด + NR-10..NR-19 ใหม่ · Data Model ไม่ถูกแตะแม้ฟิลด์เดียว ไม่มี migration ใหม่** — คำตอบของเคส "ยังไม่มี `LessonId`" คือ draft อยู่ในเบราว์เซอร์แล้ว flush ผ่าน `PUT .../narrations/{slideObjectId}` ตัวเดิมทีละหน้า *หลัง* บทเรียนเกิดและผูกไฟล์แล้ว (`LessonSlideNarrationService` ไม่ต้องแก้เลย) · **✅ CR-2.h ตัดสินแล้ว = `ScopeType = "lesson"`** (เปลี่ยนค่าที่เขียนใหม่ ไม่ใช่ schema · ไม่มี backfill · เด็คเก่าที่ scope `company` ไม่ถูกย้าย O-14) · **R4.6.3 ทำได้ด้วยของที่มี** — `PdfSlidesRenderer` รับ `Stream` ล้วน แต่ `RenderPdfPageAsync` เดิมใช้ตรง ๆ ไม่ได้ (บังคับว่าต้องมีแถว `DocumentResource`) → preview session ในหน่วยความจำ + endpoint ใหม่ 2 ตัว (NR-10/NR-11) · **UC-5 amend: "ห้ามแตะ `handlePdfUpload`" ถูก supersede แล้ว — QA ห้ามรายงานเป็นการละเมิด R8.3** · **Module I และ J แก้ `LessonForm.tsx` ไฟล์เดียวกัน ห้ามทำขนาน ลำดับ I → J (R-24)** · **🟠 `Q-J1`** (ตีความ R4.6.5 กับ byte ที่ต้องผ่าน server) **· `Q-J2`** (ภาพสไลด์ในหน้าแก้บทพูดเดิม) — **✅ ปิดทั้งคู่แล้วในรอบถัดมา ดูบรรทัดถัดไป** · **✅ 2026-08-26 (รอบที่สองของวัน · `system-analyst` amend ปิดข้อรอคำตอบอย่างเดียว): `Q-J1` = "ไม่ขัด — แตะชั่วคราวได้"** (R4.6.5 ห้าม state ที่ persist/กลับมาทำต่อได้ ไม่ใช่ห้าม byte แตะโปรเซส → **NR-10 preview session ใน `IMemoryCache` sliding 10 นาที ไม่มีแถวใน DB ไม่มีรายการในคลัง ไม่มี index ยืนตามที่ออกแบบไว้**) **· `Q-J2` = "ใส่ทั้งสองที่"** (**NR-18 ภาพสไลด์ในหน้าแก้บทพูดเดิม อยู่ในขอบเขต ⛔ ห้ามตัด**) · **ทั้งสองข้อตรงกับทางที่ `system-analyst` เสนอไว้ → ไม่มี contract / Data Model / ข้อความ NR ถูกแก้แม้คำเดียว ไม่มี migration ไม่แตะ checkbox/`plan.md`/`requirement.md`/เอกสารโมดูลอื่น ไม่มีโค้ดถูกเขียน** (แก้ `design.md` 4 จุดที่เป็น bookkeeping ล้วน: กล่องสถานะ · ช่องคำตัดสินแถว R4.6.5 · `## Unresolved Open Questions` · Change Log) · **ผลรวม: Module A–J ไม่มีข้อรอคำตอบค้างเลยแม้ข้อเดียว** · **ผู้รับต่อ: `project-manager` วาง phase ของ Module J ได้ทั้งก้อน ไม่มีเงื่อนไข/คำถามค้าง** (ข้อจำกัดที่ยังบังคับ = **R-24 I ก่อน J ห้ามทำขนาน** + 🟡 O-1..O-16 ซึ่งเป็นของที่ตั้งใจไม่ทำ ไม่ใช่ของที่รอคำตอบ) · **⛔ engineer ยังห้ามหยิบจนกว่าจะมี task ใน `plan.md`** · **✅ 2026-08-26 (`qa-engineer`, FULL รอบแรกของ Phase 9): 7/7 `plan.md` tasks (UC-1, UC-2, UC-3, UC-6, UC-7, UC-8, UC-9) ติ๊ก `[x]` ครบ — ตรวจโค้ดจริงทีละ task เทียบ `design.md` §Upload Consolidation Rules UC-1..UC-10** · `LessonForm.tsx` ไม่มีการ์ด/import `DocumentUploadList` เหลือแล้ว, `DocumentUploadList.tsx` ลบ `fixedScope`/`libraryMode`/`activeScope`/`primaryDocumentId` ออกจากโค้ดที่ทำงานจริงครบ (คำว่า `fixedScope` เหลือแค่ในคอมเมนต์อธิบายประวัติ 2 จุด ซึ่งตรง convention "คอมเมนต์อธิบายทำไม" ของ `CLAUDE.md` — **ไม่นับเป็น defect ตาม UC-2** เพราะ contract สั่งลบ props union/ตัวแปร/เงื่อนไขที่ทำงานจริง ไม่ได้ห้ามคำนี้ปรากฏใน comment ล้วน ต่างจากที่ Codex ภายนอกรายงานไว้ Minor), badge KL-7 (`slideLessonFor`) ทำหน้าที่แทน `primaryDocumentId` ที่หน้าคลังแล้วจริง, บรรทัดสรุป UC-6/UC-7/UC-8 ใน `LessonForm.tsx` อ่านอย่างเดียวจริง (ไม่มีปุ่มอัป/ลบ/ย้าย) ลิงก์พา query param `scopeType=lesson&scopeId=` ไปหน้าคลังที่อ่าน filter จาก URL แล้วจริง (`initialFilterFromSearchParams`), `lessons/new/page.tsx` ไม่ต้องแก้ตาม UC-9 ยืนยันแล้ว, `handlePdfUpload`/UC-5 ไม่ถูกแตะ (ยังเรียกจาก 3 จุดเดิม ยังสร้างบทเรียน PDF ได้), ไม่มี migration ใหม่ (`dotnet ef migrations has-pending-model-changes` clean, ไม่มีไฟล์ใหม่ใน `Migrations/`), ไม่มี endpoint ใหม่ (`api-client.ts` shape เดิม) ตรง UC-4/UC-10 · **automated checks รันเองซ้ำทั้งหมด**: frontend `typecheck`/`lint`/`test`(69/69)/`build` ผ่านหมด, backend `dotnet build` 0/0 (regression check เฉยๆ เพราะ Phase 9 ไม่มี `[backend]` task) · **พบเพิ่มระหว่างตรวจ (ไม่ใช่ scope ของรอบนี้แต่บันทึกไว้)**: **Phase 8 (Module H) มีโค้อยู่ใน working tree แล้วจริงแต่ 0/37 `plan.md` tasks ยังไม่เคยผ่าน `qa-engineer` เลยแม้รอบเดียว** — ไม่บล็อกการปิด Phase 9 รอบนี้ (Phase 9 เป็น UI ล้วน ไม่ขึ้นกับ Phase 8 ผ่าน QA แค่ขึ้นกับ Phase 8 deploy ก่อนตาม R-18) แต่บล็อก `devops` ทั้งสอง phase จนกว่าจะมี FULL round ของ Phase 8 ด้วย · **ผลคือ Phase 10 (Module J) `[frontend]` tasks ปลดล็อกแล้วตาม R-24** (เงื่อนไข "Phase 9 ผ่าน `qa-engineer`" ครบแล้ว) — รายละเอียดเต็มที่ `review.md` · **ผู้รับต่อ: `frontend-engineer` (Phase 10 `[frontend]`, ปลดล็อกแล้ว) หรือใครก็ได้ที่อยากหยิบ Phase 8 ไปเข้าคิว `qa-engineer` FULL รอบแรก** · **⚠️ 2026-08-26 (พบตอนเริ่มรอบ Phase 10 — นอก scope ของรอบนี้ แต่ต้องบันทึก): `plan.md` Phase 8 เปลี่ยนจาก 0/37 เป็น 36/37 `[x]` โดยไม่มีรอบ `qa-engineer` ใน `review.md` รองรับเลย (ไม่มี Verification Summary/manifest/Change Log ของ Phase 8 ที่ไหน) — ไม่ทราบว่าใคร/อะไรติ๊กและตรวจโค้ดจริงหรือไม่ ต้องให้ผู้ใช้ตรวจสอบก่อนเชื่อสถานะ Phase 8** · **✅ 2026-08-26 (`qa-engineer`, FULL รอบแรกของ Phase 10/Module J): 20/21 `plan.md` tasks ติ๊ก `[x]` — backend 8/8 ✅ (endpoint ใหม่ 3 ตัวของ NR-10/NR-11/NR-18, NR-5 formula parity, unit test NR-11 same-message, `API_CONTRACT.md` อัปเดต), frontend 12/13 ✅ + 1 ⚠️ Partial (`implement NR-13` — เคสล้มขั้น 1 ตั้ง `stepError` ไว้แต่ UI ไม่เคยแสดงเพราะ `commitStarted` gate บล็อกไว้ ส่งกลับ `frontend-engineer` แล้ว รอบที่ 1) · automated checks รันเองซ้ำทั้งหมด: backend `dotnet build` 0/0, `dotnet test --filter "Category!=Integration"` 269/269 ผ่าน, `dotnet ef migrations has-pending-model-changes` clean (ยืนยัน "ไม่มี migration" ตาม contract); frontend `typecheck`/`lint` clean, `test` 69/69 (ไม่มี test ใหม่ของ Phase 10 เลย — บันทึกไว้ใน `review.md` §Unverified Behaviour), `build` สำเร็จหลังล้าง `.next` cache เก่าที่ทำให้ build รอบแรก fail แบบไม่เกี่ยวกับโค้ด · พบ note เพิ่มเติมส่งให้ `security` ตรวจตอน audit (ไม่ block รอบนี้): `RenderPdfPreviewPageAsync` ไม่ห่อ `RenderPagePng` (PDFium) ด้วย try/catch ต่างจาก endpoint พี่น้องที่ห่อไว้ · **ผู้รับต่อ: `frontend-engineer` (แก้ NR-13), `security` (Phase 10 ยังติด 🔒 gate ไม่เคยตรวจเลยทั้งโมดูล), ผู้ใช้ (ตัดสินใจเรื่อง NR-13 routing + สอบถามความคลาดเคลื่อนของ Phase 8 checkbox) · **✅ 2026-08-26 (`qa-engineer`, TARGETED รอบที่ 7 — ตรวจ fix ของ P8-01/P8-02/NR-13 ที่มีคนแก้ไว้ใน working tree แล้วแต่ยังไม่เคยผ่าน QA): ทั้งสามข้อ ✅ Verified จริงด้วยการอ่านโค้ดตรง**: **P8-01** — `DocumentLibraryFilterBar.tsx` มี status `Select` แล้ว (`pending`/`indexed`/`failed`/`ทั้งหมด`) ต่อกับ `filter.status`, `DocumentUploadList.tsx`/`KnowledgeQnATable.tsx` ทั้งคู่เพิ่ม `filter.status` เข้า dependency array ของ reload effect แล้ว, backend (`GetByScope`/`GetAll`) กรอง status อยู่แล้วเดิมไม่ต้องแก้ → **KL-1 layout task (task ที่เหลืออยู่ตัวเดียวของ Phase 8) ติ๊กแล้ว → Phase 8 = 37/37** · **P8-02** — `CompanyIsolationTests.cs` เพิ่ม 2 test ใหม่ที่ยิงกับ `ApplicationDbContext` จริง (`DocumentResourceGetAllInCompanyOnlyEverSeesItsOwnCompanysDocuments`, `KnowledgeQnAGetAllInCompanyOnlyEverSeesItsOwnCompanysQnA`) seed สองบริษัทจริง ยืนยัน `GetAllInCompany()` ทั้งเอกสารและ Q&A แยกบริษัทถูกทั้งสองทิศทาง ตรง R-16 เป๊ะ — diff ยืนยันเป็นการเพิ่มล้วน ไม่แตะ test เดิม → ปิด Open Issues row นี้ (ไม่มี checkbox ให้ติ๊กเพราะไม่เคยมี task แยกของ test นี้ใน `plan.md`) · **NR-13** — `PdfLessonContentPhase.tsx` เพิ่มบล็อก `{!commitStarted && stepError && (<Alert.../>)}` ให้ error ของขั้น 1 แสดงได้แม้ `commitStarted` ยังเป็น `false` (เพราะ `lessonId` ไม่เคยถูกตั้งตอนขั้น 1 ล้ม) ปุ่ม "ยืนยันสร้างบทเรียน" ยังอยู่ในวิวเดิมกดซ้ำได้จริง → **`implement NR-13` ติ๊กแล้ว → Phase 10 = 21/21** · **ตรวจ blast radius + shared-code watchlist ครบ**: diff ของ `LessonForm.tsx`/`api-client.ts`/`domain.ts`/`LessonController.cs`/`ILessonConfigService.cs`/`LessonConfigServiceTests.cs`/narrations `page.tsx`/`SlideNarrationEditorCard.tsx` เทียบกับที่ Round 5/6 บันทึกไว้แล้ว ตรงกันทุกบรรทัด ไม่มี drift ใหม่ (diff ก้อนใหญ่เทียบกับ HEAD เป็นเพราะโค้ด Phase 10 ทั้งก้อนยังไม่เคย commit มาก่อน ไม่ใช่เพราะมีอะไรเปลี่ยนหลัง Round 5) · **เจอเพิ่ม 1 จุดนอกขอบเขต 3 ข้อที่มอบหมาย แต่แก้ถูกแล้วจริง (บันทึกไว้ ไม่มี checkbox ให้ติ๊กเพราะเป็นของโมดูล `learning-session`)**: `frontend/src/components/meeting/SlidesEmbed.tsx` เคย prefix `getApiBaseUrl()` ซ้ำสองรอบทับ `slideUrl` ที่ `api-client.ts:277` resolve เป็น absolute URL ไว้แล้ว ทำให้ภาพสไลด์ของบทเรียน PDF ทุกบทพังในห้องเรียนจริง — ยืนยันว่าตอนนี้แก้ถูกต้องแล้วด้วยการอ่านทั้งสองฝั่งของ contract · **automated checks รันเองซ้ำทั้งหมด (ทั้งโปรเจกต์ ไม่ใช่แค่ไฟล์ที่เปลี่ยน)**: frontend `typecheck`/`lint` clean, `test` 69/69 (ไม่มี test ใหม่ของทั้งสามจุดแก้), `build` clean 24 routes; backend `dotnet build -c Release` 0 warning/0 error (ดีขึ้นจาก 2 CA1416 warning เดิมของ Round 6), `dotnet test --filter "Category!=Integration"` **271/271** (220 Application รวม +2 จาก P8-02, 41 Providers, 10 Api.IntegrationTests ไม่เปลี่ยน), `dotnet ef migrations has-pending-model-changes` clean · **นี่เป็นรอบ TARGETED ไม่ใช่ FULL** — Phase 8/Phase 10 ติ๊กครบ 100% ทั้งคู่แล้ว แต่ตาม `.claude/agents/qa-engineer.md` ยังต้องมี **FULL round อีกหนึ่งรอบ** ก่อนถึงเกณฑ์ deploy ของ `devops` ได้ (ไม่ใช่เพราะเจออะไรผิดเพิ่ม) · ทั้งสอง phase ยังติด `🔒 Security gate` รอ `security` เหมือนเดิม ไม่เปลี่ยน · **ผู้รับต่อ: ใครก็ได้เรียก `qa-engineer` FULL รอบสุดท้ายของ Phase 8+Phase 10 ก่อนเข้าคิว `security`/`devops`** · **🆕 2026-08-26 (`business-analyst` · CR-3 · ไม่มีโค้ด ไม่แตะ checkbox ไม่แตะ `design.md`/`plan.md`): เพิ่ม R4.7 "ตัดหน้าออกจากบทเรียน PDF" (R4.7.1–R4.7.12) ลง `requirement.md` — PDF เท่านั้น, ตัดได้ทั้งในเฟสสร้างใหม่ (Module J) และบทเรียนที่สร้างแล้ว, หน้าที่ตัดต้องไม่ถูกสอนและใช้ตอบไม่ได้ (ถอด vector จริงแบบ R6.1 + re-index อัตโนมัติแบบ R4 ข้อ 5), เลขหน้าที่คนเห็นเรียงใหม่ 1–9, เอากลับได้เสมอพร้อมบทพูดเดิม, ต้องเหลืออย่างน้อย 1 หน้า, คนที่กำลังเรียนอยู่ไม่สะดุด (มีผลรอบถัดไป), อัป PDF ทับล้างรายการที่ตัดและนับรวมในคำเตือน NR-3, สิทธิ์ `cs`/`admin` เท่าเดิม · **เป็นการกลับมติ CR-2.k วันเดียวกันโดยเจ้าของโปรเจกต์ยืนยันเอง** (ครึ่ง "ซ่อนหน้า" กลับมติ · ครึ่ง "จัดลำดับหน้า" ยังปิดอยู่) · **ผู้รับต่อ: `system-analyst` (amend)** — งานหลักคือ amend `NR-1..NR-19`, ตัดสินผลกับ `pdf-page-N`/chunk id ใต้ **NR-4** เมื่อเลขที่คนเห็นเรียงใหม่ (คือ blast radius ที่เจ้าของโปรเจกต์กังวลเอง), กลไกถอด vector, การไม่ให้การเรียนที่เปิดอยู่สะดุด และจัดลำดับงานกับ Module J ที่ยังไม่ FULL/ไม่ audit/ไม่ deploy · ⛔ **ยังไม่ส่ง engineer** · **✅ 2026-08-26 (`system-analyst` amend รับ CR-3 เสร็จแล้ว · เจ้าของโปรเจกต์เคาะ Q-K1–Q-K4 ทีละข้อ · ไม่มีโค้ด ไม่แตะ checkbox ไม่แตะ `plan.md`/`requirement.md`): เกิด Module K (🔒 Security gate) · ⚠️ รอบนี้ Data Model ถูกแตะจริง ต่างจากรอบ CR-2 — ตารางใหม่ `DM-17 · LessonExcludedSlide` + migration `MG-K1` (additive ล้วน ไม่มีคอลัมน์ใหม่บนตารางเดิม ไม่มี backfill `Down()` ย้อนได้ 100%) · contract ใหม่ `## Page Exclusion Rules (R4.7)` EX-1..EX-12 + amend NR-1/2/3/4/5/6/7/8/12/13/15/16/17/18** — **ข้อสรุปสองข้อที่เป็นสาระของรอบนี้**: (1) **R4.7.6 ไม่ขัดกับ NR-4** เพราะ `SlideObjectId` (`pdf-page-N`) กับ `Index` เป็นคนละฟิลด์ตั้งแต่ `PdfSlidesRenderer.BuildContent:83-84` การเรียงเลขใหม่แตะเฉพาะ `Index` และ id ไม่ขยับ ซึ่งเป็นเหตุผลเดียวที่ R4.7.8 (บทพูดเดิมคืนมา) ทำงานได้ → NR-4 ไม่ถูกผ่อน มีแต่ข้อห้ามใหม่ต่อท้าย (ห้ามคำนวณเลขหน้าไฟล์จากลำดับที่แสดง — `narrations/page.tsx:143` และ `PdfLessonContentPhase.tsx:309` ต้องแก้ทั้งคู่) (2) **หน้าหนึ่งของเด็คมี vector สองใบไม่ใช่ใบเดียว** (`pdf-page-N` ของบทพูด + `{documentId}-page-N` ของเอกสารเด็ค ตาม NR-8 หลังมติ CR-2.h) → **ถอดใบเดียว = R4.7.3(ข) ล้มทั้งข้อ** และเด็คเก่าที่ scope `company` (O-14) จะยังตอบได้ทั้งบริษัท · แก้ได้ด้วยเครื่องมือที่ R6.1 ทำไว้แล้วทั้งหมด (`DeleteVectorsAsync` + job `vector_delete` + `DocumentChunk.VectorId`/`.NamespaceKey`/`.Text`) **ไม่มีกลไกใหม่** · **มติที่เคาะ**: Q-K1 = ตารางใหม่ (ตัด "ธงใน `SlideConfigs`" ทิ้งเพราะ `SaveAsync` เขียนทับทั้งก้อนทุกครั้ง) · Q-K2 = รีเฟรชแล้วได้ชุดหน้าใหม่ทันที ไม่ snapshot ต่อ session **และถอด vector ทันทีโดยยอมให้ตอบ `not_found` ระหว่างที่แท็บเก่ายังสอนหน้านั้นอยู่ (R-25 — `qa-engineer` ต้องไม่รายงานเป็น ❌)** · Q-K3 = หน้าแก้บทพูดแสดงครบทุกหน้า การ์ดสีจาง + ปุ่มเอากลับ + ช่องบทพูด read-only (server ต้องปฏิเสธด้วย ไม่ใช่แค่ UI) · **Q-K4 = แยกเป็น Module K และเริ่มหลัง Module J ปิดรอบ FULL เท่านั้น (R-26)** · **กับดักสองข้อที่เขียนไว้ให้ engineer แล้ว**: `ProcessLessonIndexAsync` `.Where(resolvedById.ContainsKey)` จะดรอปหน้าที่ถูกตัดทิ้งจน vector ไม่เคยถูกลบโดยงานขึ้น `succeeded` ตามปกติ (NR-6) · ลำดับภายในขั้นที่ 3 ของ NR-12 ต้องเป็น ล้างตาม NR-3 → เขียน exclusion → index (สลับ = หน้าที่ CS ตัดทั้งเฟสหายเงียบ ๆ) · **`GET {id}/narrations/count` เปลี่ยนรูปเป็น `{ count, excludedCount }` — breaking แต่ caller เดียว** · **ผู้รับต่อ: `project-manager`** (วาง phase ของ Module K โดยเคารพ R-26 — K ต่อจาก J ห้ามคร่อม) · ⛔ **ยังไม่ส่ง engineer** · **🆕 2026-08-26 (`business-analyst` · CR-5 · ไม่มีโค้ด ไม่แตะ checkbox ไม่แตะ `design.md`/`plan.md`): เพิ่ม R10 "จังหวะยืนยันของ flow สร้างบทเรียน PDF และกล่องยืนยันของหน้าจอ admin" (R10.1–R10.9) — (ก) ห้ามพาผู้ใช้ออกจากหน้าเองเมื่อสร้างสำเร็จ ต้องกดยืนยันที่จอสรุปผล และปลายทางเปลี่ยนเป็น **หน้ารายการบทเรียนหลัก**, ทั้งช่วงรันและช่วงสรุปอยู่ใน modal ที่ **ปิดไม่ได้ระหว่างรัน** (Esc/คลิกนอกไม่ได้), ระหว่างรันแสดง **รายการขั้นตอนติ๊กทีละขั้น** ไม่ใช่ spinner เปล่า, ล้มเหลวยังจบในกล่องเดิมโดย retry รายขั้น + ลิงก์ทางออกของ NR-13 ต้องอยู่ครบ, คำเตือน NR-15 **แยกกล่องเหมือนเดิม ห้ามยุบรวม** · (ข) เลิกใช้ `window.confirm` ครบ **5 จุด** (ลบหมวดย่อย `CategoryFormDialog.tsx:130` · ลบเอกสาร `DocumentUploadList.tsx:244` · reset demo `lessons/page.tsx:94` · ลบหมวด `:116` · ย้ายบทเรียนเข้าถัง `:144`) โดย **ไม่เปลี่ยนข้อความ/เงื่อนไข/สิทธิ์/ผลลัพธ์** · **⚠️ ไม่ใช่งานผิวหน้า: R10.1/R10.2 กลับมติ NR-12 ท้ายข้อ** (ที่ผูกไว้ว่า "สำเร็จครบ 4 ขั้น → `router.push` ไปหน้าแก้บทเรียนของ slug นั้น") **ทั้งเงื่อนไขและปลายทาง** — เจ้าของโปรเจกต์ยืนยันว่าตั้งใจ (CR-5.c) · ลำดับ 4 ขั้นและข้อห้ามสลับ 3↔4 **ห้ามถูกแตะ** CR นี้แตะเฉพาะสิ่งที่เกิดหลังขั้น 4 สำเร็จ · **ค้น `window.confirm` ทั้ง `frontend/src` แล้วทั้ง 5 จุดอยู่ในหน้าจอ `knowledge-base` ทั้งหมด** ไม่มีใน `company-admin`/`learning-session` → CR โฟลเดอร์เดียว ไม่ต้องแตกข้ามโมดูล · **🐞 อาการ "ไม่พบบทเรียนนี้ค่ะ" หลัง redirect (slug ภาษาไทย) = นอกขอบเขต CR-5** ให้ `system-analyst`/`qa-engineer` ตรวจแยกหลัง R10 ถูกออกแบบ **ห้ามถือว่า R10 คือการแก้อาการนี้** · **ผู้รับต่อ: `system-analyst` (amend NR-12 ท้ายข้อ + NR-13 + ตัดสิน mechanics ของกล่องยืนยัน 5 จุด)** · ⛔ **ยังไม่ dispatch และยังไม่ส่ง engineer** · **✅ 2026-08-26 (`system-analyst` amend รับ CR-5 เสร็จแล้ว · ไม่มีโค้ด ไม่แตะ checkbox ไม่แตะ `plan.md`/`requirement.md`/เอกสารโมดูลอื่น): เกิด Module M (ไม่ติด 🔒 gate · frontend ล้วน) · Data Model ไม่ถูกแตะแม้ฟิลด์เดียว ไม่มี migration ไม่มี endpoint ใหม่ ไม่มีงาน `[backend]` แม้ task เดียว** — **NR-12 ท้ายข้อถูกกลับมติและแยกเป็นสองสัญญาที่ NR-21** (เงื่อนไข: ต้องให้ CS กดยืนยันเอง · ปลายทาง: `/admin/lessons`) โดยประโยคเดิมถูกขีดฆ่าคาไว้พร้อมป้ายห้าม implement **และลำดับ 4 ขั้น/ข้อห้ามสลับ 3↔4 ไม่ถูกแตะ (ยืนยันจากโค้ดจริง)** · **contract ใหม่ `NR-20..NR-24` + `## Confirmation Dialog Rules` `CD-1..CD-10`** · `NR-13`/`NR-15` amend เฉพาะจุด · **สามข้อค้นพบจากโค้ดที่ทำให้ CR นี้ห้ามลัดไป engineer**: `router.push` อยู่ใน `flushNarrations` (`PdfLessonContentPhase.tsx:126-128`) และถูกเรียกจากเส้นทาง retry ด้วย → แก้แค่ปลายทางจะเหลือ auto-navigate ที่การลองซ้ำ (R-32) · ล้มขั้น 1 ไม่เคยเซ็ต `lessonId` → ถ้าผูก `open` ของ modal กับ `commitStarted` ความล้มเหลวที่พบบ่อยที่สุดจะไม่มีกล่องเลย (R-33) · `DialogContent` มี `showCloseButton` default `true` ซึ่งเป็นทางปิดที่ R10.5 ห้าม · **คำตอบของความตึง R10.5↔R10.6: กล่องไม่เคยถูก "ปิดทิ้ง" เลย — ตอนล้มเหลว CS ออกได้ผ่านลิงก์ทางออกของ NR-13 (ขั้น 2–4) หรือปุ่ม "กลับไปแก้ข้อมูลบทเรียน" (ขั้น 1 เท่านั้น) ซึ่งเป็นการรักษาสัญญาเดิมของ NR-13** · **✅ `Q-M1` ปิดแล้ว 2026-08-26 — เจ้าของโปรเจกต์เคาะทางเลือก (ก) ล็อก modal ครบทุกสถานะ ออกได้เฉพาะปุ่มในกล่อง (Esc/คลิกนอกปิดไม่ได้เลย ทั้งตอนรัน สำเร็จ และล้มเหลว)** — ตรงกับที่ `system-analyst` เคาะไว้จากตัวบทและเขียนลง NR-23/NR-24 แล้ว **จึงไม่มีข้อความ contract ข้อใดถูกแก้ ไม่มี Data Model ไม่มี migration ไม่แตะโค้ด** เปลี่ยนแค่สถานะจาก "ข้อสันนิษฐาน" เป็น "มติที่บังคับใช้" · **ไม่มีข้อรอคำตอบค้างในโมดูลนี้แล้วแม้ข้อเดียว** · **`qa-engineer` ต้องตรวจสามจุดจากมตินี้ให้ครบ ไม่ใช่จุดเดียว**: ตารางสถานะของ NR-24 · ข้อห้ามผูก `open` ของ modal กับ `commitStarted`/`lessonId` (NR-20) · `showCloseButton={false}` (NR-23) — พลาดข้อสุดท้ายข้อเดียวก็เปิดทางปิดที่มตินี้ห้ามไว้ทั้งข้อ · **`O-19` อาการ "ไม่พบบทเรียนนี้ค่ะ" = deferred ห้ามอ้างว่ารอบนี้แก้แล้ว (R-36)** · **⛔ ลำดับงาน: ห้ามคร่อม Phase 12 (Module L)** — พบว่าโค้ด Module L มีอยู่จริงแล้วบางส่วนทั้งที่ `plan.md` Phase 12 ยัง `[ ]` ทุกช่อง (R-35 · โค้ดชนะ index) · **R-34: Phase 10 ปิด FULL แล้วแต่ยังไม่ deploy และรอบนี้แก้โค้ดในเฟสนั้น** → manifest ของ Round 8 ไม่ตรงแล้ว และไม่ควร deploy Phase 10 ก่อน Module M · **ผู้รับต่อ: `project-manager` (วาง phase ของ Module M ได้ทั้งก้อน ไม่มีเงื่อนไข/คำถามค้าง — ต้องเขียนข้อห้ามคร่อม Phase 12 ลงหัวข้อ phase ด้วย)** · ⛔ **ยังไม่ส่ง engineer** |
...
- Phase 1 (Module A: ตาราง `AuditLog` + interceptor) — implemented ✅ · verified ✅ (FULL) · security ⬜ · deployed ⬜
- Phase 2 (Module B: 5 จุด raw-SQL) — implemented ✅ · verified ✅ (FULL) · security ⬜ · deployed ⬜

**✅ `system-analyst` — `design.md` เสร็จแล้ว:** ปิดมติ 8 ข้อกับเจ้าของโปรเจกต์ (OQ-2 `CompanyId`
nullable + ไม่มี query filter · OQ-3 ไม่บันทึก action ที่ระบบก่อ · OQ-4 ตัด login ออก · Q-A1
`AuditLog` ไม่ implement `IEntityMaster` (append-only) · Q-A2 ไม่มี `MetadataJson` · Q-A3 log ล้ม =
business write ล้มตาม (atomic) · OQ-5 `cs` ไม่เห็น log · Q-A4 ยืนยันทุกแถวจริง) · Data Model
**DM-A1..DM-A5** — ตารางใหม่ `AuditLog` 7 ฟิลด์ + `AuditAction` constants + 3 index + ctor ของ
`ApplicationDbContext` + signature ของ `AccelerateLessonPurge` · **additive ล้วน ไม่มี breaking
change ต่อข้อมูล** · สัญญา **AU-1..AU-20** (interceptor ผ่าน `SaveChanges(bool)` override) และ
**RS-0..RS-8** (5 จุด raw SQL ที่ต้องเขียน log ด้วยมือ: `TryArchive`, cascade `TrainingLink`,
`TryRestore`, `TryRestoreAndCancelPurge`, `AccelerateLessonPurge` + 3 จุดที่จงใจไม่เขียน) ·
แบ่งเป็น **Module A** (ตาราง + interceptor) และ **Module B** (5 จุด raw SQL) **ทั้งคู่ติด
🔒 Security gate** · ความเสี่ยง AR-1..AR-9 (ข้อใหญ่สุด: AR-4 `ExecuteUpdate` จุดใหม่ในอนาคตจะหลุด
log เงียบๆ)

**✅ 2026-08-28 `system-analyst` amend — ปิด OQ-A1 (เอกสารล้วน ไม่มีโค้ด ไม่มี migration):**
มติ = **ทางเลือก ก · `CompanyId` ของแถว log ที่ entity เป็น `AdminUser` เป็น `null` เสมอ
(ระดับระบบ)** ยืนยันตามคำตอบเดิมของ OQ-2 · **AU-11 ที่เขียนเป็น default ไว้ถูกต้องอยู่แล้ว —
ไม่มีการแก้ Data Model / AU-* / RS-* / Blast Radius / Migration Plan / การแบ่ง Module แม้จุดเดียว**
(แก้เฉพาะ bookkeeping 6 จุด: กล่องสถานะ · แถว R3/R3.1 · ตารางมติ · AU-11 · AR-10 ใหม่ ·
`## Unresolved Open Questions` + Change Log) · **AR-10 ใหม่บันทึก trade-off ที่ยอมรับแล้ว:**
dashboard ในอนาคต — `admin` ระดับบริษัทจะไม่เห็นว่าใครแก้/ปิดบัญชีพนักงานในบริษัทตัวเอง
(เห็นได้เฉพาะ `owner` ที่มองแถวระดับระบบ) ถ้าจะเปลี่ยน = กลับมติ ต้องผ่าน `system-analyst` ·
**ไม่มีข้อรอคำตอบเหลือในโมดูลนี้แล้วแม้ข้อเดียว** — OQ-A2 (action `restore`) / OQ-A3
(`ActorRole` snapshot) / OQ-A4 (ทบทวน retention) เป็นข้อที่ **เลื่อนโดยตั้งใจ** ทั้งหมด ทำทีหลัง
เป็น additive ไม่มีต้นทุนจม

**✅ 2026-08-28 `project-manager` — `plan.md` เสร็จแล้ว:** 2 phase ตรงกับ 2 Module
(Phase 1 = Module A, Phase 2 = Module B, Phase 2 ขึ้นกับ Phase 1) ทั้งคู่ `🔒 Security gate` ·
งานทั้งหมดเป็น `[backend]` (R6 ตัด UI/endpoint) · เพิ่มงานเทสต์ตาม AU-20 (AU-2/AU-4/AU-5/AU-6/
AU-9 — AU-15 ทดสอบด้วย InMemory ไม่ได้) · **ทุก phase ประกาศบล็อก C1 ไว้ที่หัวไฟล์และหัว phase**

**✅ 2026-08-28 ไฟเขียว C1 ปลดล็อกแล้ว:** เจ้าของโปรเจกต์ยืนยันเองในแชทว่าให้ไฟเขียว migration
("ให้ไฟเขียวตอนนี้เลย") — แก้ banner หัวไฟล์และหัว phase ใน `plan.md`, `design.md`, `requirement.md`
ทั้งสามไฟล์แล้ว (⛔ → ✅)

**✅ 2026-08-28 `backend-engineer` — Phase 1 (Module A) implement เสร็จแล้ว:** สร้าง
`AuditLog.cs` (7 ฟิลด์ ไม่ implement `IEntityMaster` ตาม DM-A1) + `AuditAction.cs` (static
class/const string) · แก้ `ApplicationDbContext.cs` เพิ่ม `ICurrentUser` ใน ctor + `DbSet<AuditLog>`
+ 3 index ไม่มี query filter (DM-A3) + override `SaveChanges(bool)`/`SaveChangesAsync(bool, ct)`
เป็น interceptor ผ่าน `BuildAuditLogRows()` (AU-1..AU-16 ครบ ไม่มี `try/catch` กลืน exception ตาม
AU-15) · สร้าง migration `AddAuditLog` ด้วย `dotnet ef migrations add` แล้ว (**ยังไม่ได้รัน
`database update` กับ database ใดๆ** — รอ `devops`) · แก้ blast radius ครบ 3 จุด
(`DesignTimeDbContextFactory.cs`, `CompanyIsolationTests.cs`, และ
`ModuleLRepositoryIsolationTests.cs` ที่ compiler เจอเพิ่มนอกเหนือจากที่ `plan.md` ระบุไว้) ·
อัปเดตเอกสาร schema ครบ 5 ไฟล์ (16→17 ตาราง) · เพิ่ม 8 tests ตาม AU-20 (AU-2/AU-4/AU-5/AU-6 ครบ
3 branch/AU-9) — **AU-15 (atomic rollback) ยังไม่มีเทสต์ เพราะ EF InMemory ไม่มี transaction จริง
บันทึกเป็น Unverified Behaviour ให้ `qa-engineer` ตามที่ออกแบบไว้** · `dotnet build` 0 error,
`dotnet test --filter "Category!=Integration"` **358/358 ผ่าน** · ไม่มีการสร้าง
repository/service/DTO/controller/endpoint สำหรับ `AuditLog` ตาม AU-17/R6

**✅ 2026-08-28 `backend-engineer` — Phase 2 (Module B) implement เสร็จแล้ว:** แก้ signature
`IBackgroundJobRepository.AccelerateLessonPurge` เพิ่ม `actorUserId` (DM-A5) · เขียน `AuditLog`
ด้วยมือครบ RS-1..RS-5 ทั้งหมด (`TryArchive` 1 แถว + cascade `TrainingLink` 1 แถวต่อ id, `TryRestore`
เปิด transaction ใหม่ + 1 แถว, `TryRestoreAndCancelPurge` 2 แถว, `AccelerateLessonPurge` เปิด
transaction ครอบ + 1 แถว) · แก้ call site `ILessonConfigService.cs:502` + fake ทดสอบ
(`ServiceTestFakes.cs`) + call site เดิมอีก 2 จุดที่ compiler เจอเพิ่ม
(`LessonTrashServiceTests.cs`, `ModuleLRepositoryIsolationTests.cs`) · เพิ่มกติกาถาวร RS-8 ลง
`CLAUDE.md` §Architecture Rules (บังคับเขียน `AuditLog` คู่กับ `ExecuteUpdate`/`ExecuteSqlRaw`/
`ExecuteDelete` ใหม่ทุกจุดในอนาคต) · RS-6/RS-7 ไม่ถูกแตะตามสัญญา · `dotnet build` 0 error,
`dotnet test --filter "Category!=Integration"` **358/358 ผ่าน**

**⚠️ Unverified Behaviour ใหม่ที่ `qa-engineer` ต้องบันทึก**: RS-1..RS-5 ทั้ง 5 จุดทดสอบด้วย EF
InMemory ไม่ได้เลย (ยืนยันแล้วว่า InMemory throw ทันทีที่เรียก `ExecuteUpdate`/`ExecuteSqlRaw`) —
มีแค่ integration test บน Postgres จริง (`ModuleLRepositoryIsolationTests.cs`, `Category=Integration`)
ที่ยืนยัน company isolation แต่**ไม่ได้ assert ว่ามีการเขียนแถว `AuditLog` จริง** ความถูกต้องของการ
เขียนแถวใน RS-1..RS-5 ตอนนี้ยืนยันด้วยการอ่านโค้ดเทียบสัญญาเท่านั้น ยังไม่มี automated test คุม

**✅ 2026-08-28 `qa-engineer` Round 1 — FULL, Phase 1 (Module A) + Phase 2 (Module B) พร้อมกัน,
28/28 verified:** รอบ QA แรกของโมดูลนี้ ตรวจทุก task จากศูนย์เทียบ Data Model DM-A1..DM-A5,
สัญญา AU-1..AU-20, RS-0..RS-8 ทีละบรรทัดกับโค้ดจริง (ไม่เชื่อรายงานของ `backend-engineer`) —
ไม่พบ ⚠️ Partial หรือ ❌ Failed แม้รายการเดียว ติ๊ก `[x]` ครบ 28/28 ใน `plan.md`. ยืนยันซ้ำด้วยการรัน
จริง: `dotnet build` 0 error, `dotnet test --filter "Category!=Integration"` **358/358 ผ่าน**
(ตรงกับที่ engineer รายงาน), `dotnet ef migrations has-pending-model-changes` ไม่มี drift,
`dotnet ef migrations list` ยืนยัน `AddAuditLog` ยังเป็น `(Pending)` — **ยังไม่เคยรัน
`database update`**. บันทึก `## Unverified Behaviour` 2 รายการใน `review.md` ตามที่ออกแบบไว้ล่วงหน้า
(AU-15 atomic rollback, RS-1..RS-5 log-in-transaction) เพราะ EF InMemory พิสูจน์ไม่ได้จริง —
`ModuleLRepositoryIsolationTests.cs` (Postgres จริง) ยืนยัน company isolation ครบแต่ไม่ assert
การเขียนแถว `AuditLog`. ไม่พบ schema drift ระหว่าง `design.md` กับ `ApplicationDbContext`/migration
แม้จุดเดียว. **ยอมรับตามเงื่อนไข autonomous FULL-round all-✅ exception ไม่ต้องหยุดรอ
AskUserQuestion.**

**Now**: พร้อมส่ง `security` audit ทั้งสอง phase เมื่อผู้ใช้สั่ง (ไม่ auto-chain) — `🔒 Security gate`
ยังเปิดอยู่ทั้งคู่ ยังไม่เคยถูก audit
**Blocked on**: `🔒 Security gate` ทั้งสอง phase (Module A: personal data ไม่มีวันถูกลบ + ไม่มี
company query filter + แตะ `SaveChanges` ทั้งระบบ; Module B: เส้นทาง archive/restore/
permanent-delete) — ต้องผ่าน `security` audit ก่อนส่ง `devops` รัน migration จริง (migration
`AddAuditLog` ยังไม่ได้ apply กับ database ใดๆ)

## knowledge-base

**✅ 2026-08-27 `qa-engineer` Round 20 — FULL, Phase 13 (Module M) ปิดสุดท้าย 14/14:** ตรวจทุก task จากศูนย์เทียบ NR-20..NR-24/CD-1..CD-10. ยืนยัน modal มี state อิสระ/ล็อก Esc-click outside-X ครบ, button matrix ทุก failure step ถูกต้อง, navigation มีจุดเดียวจากปุ่มสำเร็จไป `/admin/lessons`, step-3 retry ยังส่ง excluded ids; confirm dialog 5 จุดใช้ `AlertDialog`/payload state/ข้อความและ destructive variant ตรง contract, permission/error/busy/trash refresh เดิมไม่หาย; sweep ทั้ง `frontend/src` ไม่มี live `window.confirm/alert/prompt`; protected dialogs และ shared exports ของ `DocumentUploadList` ตรง manifest. Frontend typecheck/lint/test 82/82/build 24 routes ผ่าน. Phase 13 ไม่ติด Security gate และปิด QA บน FULL round แล้ว. Module โดยรวมยังรอ security ของ Phase 12 และเฟส gated อื่นก่อน devops.

**✅ 2026-08-27 `qa-engineer` Round 19 — TARGETED, Phase 12 (Module L) ปิด functional 31/31:** เจ้าของโปรเจกต์อนุมัติให้เดินงานต่อเกินเพดาน re-check แล้ว. ตรวจ task 528/531 จากโค้ดจริง: PostgreSQL integration test ใหม่ใช้ production `ApplicationDbContext`/repositories/raw SQL ยิง company A ไปยังข้อมูล company B ครบ `TryArchive`/`TryClaimPurge`/`TryRestore*`/cancel/accelerate และ Module L `IgnoreQueryFilters()` reads, ทุกจุดกระทบ 0 แถวและยืนยันข้อมูล B ไม่เปลี่ยน; complete-graph purge test seed narration/excluded slide/document+chunk/Q&A+source+conflict/link/session/question ครบ ยืนยัน exclusion มีอยู่ก่อน source delete และตรวจ removals/retentions/vector/storage ครบ. QA rerun: build 0/0, worker 17/17, PostgreSQL isolation 1/1, non-integration 351/351, EF model clean. ติ๊ก 528/531 กลับ `[x]`; Phase 12 = 31/31. **ยังติด `🔒 Security gate` ก่อน devops.** ลำดับที่เจ้าของอนุมัติ: QA ต่อ Phase 13 FULL แล้วจึงเรียก security.

**⚠️ 2026-08-27 `qa-engineer` Round 18 — FULL, Phase 12 (Module L) reopened at 29/31 ✅ + 2 Partial:** ตรวจครบทั้ง 31 task จากศูนย์เทียบ R9, DM-2/DM-11/DM-15/DM-16/DM-18, MG-L1 และ LT-1..LT-24. Production code ยังไม่พบ cross-company leak หรือ SQL injection: ทุก scoped `IgnoreQueryFilters()` ใส่ `CompanyId` กลับใน predicate เดียวกัน, worker resolve `job.CompanyId` ก่อน query, raw SQL ใช้ EF parameters. แต่ FULL round พบว่า Round 16 ปิด test coverage เร็วเกินไป 2 จุด: **task 528** ใช้ fake repository ทดสอบ raw-SQL tenant guard ทั้ง 4 จุด จึงไม่ได้ execute production repository/SQL จริง (`CompanyIsolationTests.cs` ยอมรับเองว่าไม่มี PostgreSQL harness); **task 531** ไม่มี successful-purge test ที่ seed/assert การลบ dependency graph ให้ครบ narration + lesson Q&A/source/conflict + document/chunk ในขณะที่ยืนยัน retention ทั้งชุด. เปลี่ยน checkbox 528/531 กลับเป็น `[ ]`; ที่เหลือ 29 ข้อยังคง `[x]`. Automated checks: frontend typecheck/lint/test 82/82/build ผ่าน; backend Release build 0/0, non-integration 350/350, EF model clean; FULL backend 353 pass/11 fail เฉพาะ external Google/provider credentials/network เดิม ไม่ใช่ Module L. **ถึงเพดาน re-check ของ coverage family เดิมแล้ว — หยุดรอเจ้าของโปรเจกต์ตัดสินใจ routing, ไม่ auto-dispatch.** `🔒 Security gate` ยังเปิดแยกต่างหาก. รายละเอียดอยู่ `_docs/module/knowledge-base/review.md`.

**✅ 2026-08-27 `qa-engineer` Round 17 — TARGETED, Phase 13 (Module M) ปิดจริงแล้ว 14/14 ✅ (TARGETED):** ตรวจ 2 task สุดท้ายที่ Round 13 บล็อกไว้ (CD-5 จุดที่ 5 `handleArchiveLesson` + sweep CD-1 สุดท้าย) หลัง Phase 12 ปิดรอบ QA แล้ว (Round 16, 31/31) ปลดล็อกให้ทำได้ — TARGETED ถูกต้องตามธรรมเนียมเพราะ Round 13 เป็น FULL ที่มี manifest บันทึกไว้ ตรวจ CD-5 จุดที่ 5 จริงจากโค้ด: state `pendingArchiveLesson` เก็บ lesson object ตรง (ไม่ใช่ closure), แยกฟังก์ชันเปิดกล่อง/ทำงานจริงถูกต้อง (ไม่มี Promise/ref wrapping), หัวเรื่อง/ข้อความ/ปุ่ม/variant ตรงตาราง CD-5 แถวที่ 5 ของ `design.md` คำต่อคำ, ใช้ pattern `AlertDialog` inline เดียวกับอีก 4 จุด (ไม่ใช่ pattern พิมพ์ยืนยันของ `LessonPermanentDeleteDialog`), การเรียก API/`setBusyLessonId`/`setTrashRefreshToken`/`setError` ยังเหมือนเดิมทุกอย่างตาม CD-8 (เทียบกับพฤติกรรมก่อนแก้), และ role gate `CAN_ARCHIVE_LESSON` ใน `CategoryTree.tsx` ไม่ถูกแตะเลย (stat ตรงกับ manifest เดิมทุกไบต์). ตรวจ sweep CD-1 เองอีกครั้งด้วย grep ทั้ง `frontend/src` พบ `window.confirm/alert/prompt` เหลือ 0 จุดจริง (มีแต่คอมเมนต์) ไม่เชื่อรายงานของ engineer, และ stat เทียบไฟล์ dialog อีก 4 จุด + ไฟล์ป้องกัน CD-9 ทั้ง 4 ตัวกับ manifest ของ Round 13 พบตรงกันทุกไบต์/บรรทัด ไม่มีจุดใดถูกแตะพลาด. ตรวจ shared-code watchlist ทั้งหมด (ไฟล์ backend ที่ working tree แสดงว่าแก้ ก็เป็นของ Phase 12 เดิมที่ตรง manifest ทุกไบต์ ไม่ใช่การแก้ใหม่). ติ๊ก checkbox ที่เหลือทั้งสองใน `plan.md` — **Phase 13 ปิดจริง 14/14 [x]**. Automated checks ทั้งโปรเจกต์: frontend typecheck/lint clean, test 82/82 (12 files), build clean 24 routes; backend Release build 0/0, non-integration test 350/350 (เท่า Round 16 เป๊ะ ยืนยันไม่มี backend regression). **กู้คืนหัวข้อ `## Unverified Behaviour — undeployed phases` ใน `review.md` ที่หายไปตั้งแต่ก่อน Phase 12 เริ่ม** (ครั้งสุดท้ายที่มีคือ Phase 13 Round 13 เอง ก่อนถูกย้ายเข้า archive ทั้งก้อน) — ดึงกลับมาจาก `review/phase-13.md` แล้วอัปเดตส่วนของ Phase 13 ให้ตรงกับ CD-5 จุดที่ 5 ที่เพิ่งปิด. ไม่ติด 🔒 Security gate (ยืนยันซ้ำ). **Phase 13 ยังไม่ deploy-eligible — รอบปิดนี้เป็น TARGETED ต้องมีอีกหนึ่งรอบ FULL ก่อนส่ง `devops`** เหมือนที่ Phase 12 เจออยู่. รายละเอียดเต็มที่ `_docs/module/knowledge-base/review.md` (current round).

<details>
<summary>ประวัติ Round 16 (TARGETED, Phase 12 ปิด 31/31 — ก่อนถูกปิดใน Round 17)</summary>

**⚠️ 2026-08-27 · `qa-engineer` Round 16 — verified ✅ (TARGETED), Phase 12 (Module L) — re-check of P12-08's three narrowed sub-gaps, Phase 12 now closed 31/31:** TARGETED round หลัง Round 15 (FULL, มี manifest บันทึกไว้) — ตรวจเฉพาะ P12-08's 3 sub-gap ที่แคบแล้ว + blast radius (ไฟล์ที่แก้) + shared-code watchlist + typecheck/lint/build/test ทั้งโปรเจกต์ ไม่ได้ตรวจ 28 task ที่ปิดแล้วใน Round 15 ซ้ำจากศูนย์. ยืนยันทั้ง 3 sub-gap ปิดจริงด้วยการอ่านโค้ดจริง ไม่เชื่อรายงานของ engineer: (1) two-company test ของ 4 raw-SQL method ที่ fake-repository level — อ่าน comment ที่ `CompanyIsolationTests.cs` บรรทัด 417-426 ยืนยันเหตุผล (EF InMemory throw บน `ExecuteSqlRaw`) เป็นจริง แล้วเทียบ predicate ของ fake ทั้ง 4 ตัวใน `LessonTrashServiceTests.cs` กับ `WHERE`/`ExecuteSqlRaw` จริงใน `ILessonConfigRepository.cs`/`IBackgroundJobRepository.cs` ทีละคอลัมน์ — ตรงกันเป๊ะ ทั้งฝั่งปฏิเสธและฝั่งที่บริษัทเจ้าของทำสำเร็จ; (2) shared-PDF-guard test สร้างสอง lesson อ้าง `PdfDocumentResourceId` เดียวกันจริง purge ใบเดียวแล้วยืนยัน resource/chunk/storage key/vector รอด — ไล่โค้ด guard จริงใน `IBackgroundJobProcessor.cs` ยืนยันเป็น path เดียวกัน; idempotent-missing-delete test ใช้ `LocalDocumentStorageProvider` จริง (ไม่ mock) กับ key ที่ไม่เคยอัปโหลดจริง ยืนยัน `AttemptCount` ไม่ขยับ (ไม่ retry); (3) `GetQueue_TrashedLessonHidesItsQuestions_RestoreBringsThemBack` ใน `KnowledgeQnAServiceTests.cs` ยืนยัน lesson id ที่ trash เชื่อมกับ question จริงผ่าน helper เดิม ไม่ใช่ coincidence, และ `GetQueue()` จริงเรียก `GetTrash()` ตามที่ test ตรวจ; finalization retention เพิ่ม assertion `TrainingLink`/`LearningSession`/`BackgroundJob` รอด + test ใหม่ยืนยัน `LessonExcludedSlide` ถูก hard-delete จริงผ่าน `excludedSlideRepository.Delete()`. ไม่มีไฟล์ test ใหม่เกิดขึ้น (Glob ยืนยัน 30 ไฟล์เท่าเดิม) แก้อยู่ใน 3 ไฟล์เดิม (`LessonTrashServiceTests.cs`/`LessonPurgeWorkerTests.cs`/`KnowledgeQnAServiceTests.cs`); ไฟล์อื่นทั้งหมดในและนอก manifest ตรวจแล้ว byte/line เท่าเดิมทุกไฟล์ ไม่มี blast radius เข้า shared code. ติ๊ก checkbox 528/530/531 ครบ — **Phase 12 ปิด QA แล้วจริง 31/31 [x], 0 Partial, 0 Failed** (P12-08 ผ่านในรอบ re-check ที่ 2 ก่อนถึงเพดาน escalation). Automated checks: frontend typecheck/lint clean (ไม่มีไฟล์ frontend เปลี่ยนตั้งแต่ Round 15 จึงไม่รัน test/build ซ้ำ — Round 15 บันทึก clean ไว้แล้ว); backend Release build 0/0, non-integration test **350/350** (+8 จาก test ใหม่ของ 3 sub-gap). **Phase 13 สอง task สุดท้าย (`handleArchiveLesson` + final CD-1 sweep) ปลดล็อกแล้ว ส่งให้ `frontend-engineer` ได้.** **แต่ Phase 12 ยังไม่ deploy-eligible ด้วยสองเหตุผลอิสระกัน: (1) รอบล่าสุดเป็น TARGETED ไม่ใช่ FULL — ตามธรรมเนียมของโปรเจกต์นี้ต้องมีอีกหนึ่งรอบ FULL ก่อนส่ง `devops` แม้ผลจะเป็น all-✅ แล้วก็ตาม (แก้ไขจากคำสั่งเดิมของรอบนี้ที่ขอให้ประกาศ deploy-eligible ไปเลย — ยึดธรรมเนียมที่ตายตัวของโปรเจกต์แทน) (2) 🔒 Security gate ยังไม่เคย audit เลย เป็นตัวบล็อกแยกจากรอบ QA.** รายละเอียดเต็มอยู่ `_docs/module/knowledge-base/review.md`; Round 15 ย้ายไปเก็บใน `_docs/module/knowledge-base/review/phase-12.md` แล้ว. **Now: เสนอให้รัน QA รอบ FULL อีกครั้ง (ควรถูกๆ เพราะไม่มีอะไรเปลี่ยนนอกจาก 3 ไฟล์ test ของรอบนี้) แล้วเรียก `security` audit ก่อนพิจารณา `devops`.**

</details>

<details>
<summary>ประวัติ Round 15 (FULL, Phase 12 second QA round — 28/31 verified, ก่อนถูกปิดใน Round 16)</summary>

**⚠️ 2026-08-27 · `qa-engineer` Round 15 — verified ⚠️ (FULL), Phase 12 (Module L) — second QA round, 28/31 verified, 3 Partial, 0 Failed:** ตรวจครบ 31 task จากศูนย์อีกครั้ง (ไม่ใช่แค่ 8 จุดที่ backend-engineer แก้) เทียบ R9, DM-2/DM-11/DM-15/DM-16/DM-18, MG-L1 และ LT-1..LT-24 พร้อม watchlist ของ shared code. ยืนยันด้วยโค้ดจริงว่า P12-01..P12-07, P12-09 และฝั่ง frontend P12-06/P12-10 **ปิดสนิทแล้ว**: `TryArchive`/`TryClaimPurge`/`TryRestore`/`TryRestoreAndCancelPurge` เป็น conditional `UPDATE`/transaction เดียวจริง (ไล่ race แบบสองคำขอพร้อมกันแล้วผู้แพ้ได้ `NotFound`/409 ไม่ใช่แค่มี `WHERE`), `GetTrash()` มี `EnsureCanArchiveOrRestore()` แล้ว, resume/progress/TTS/question/content ทุกเส้นผ่าน `GetEntityByTokenForContentAccess`/`GetInProgressEntityByLearnerKey` เท่านั้น (ไล่อ่าน `ILearningSessionService.cs`/`ITrainingLinkService.cs` เต็มไฟล์ยืนยันไม่มี logic หลุด/พังจาก brace bug เดิม), `lesson_purge` retry ทุก 24 ชม.ไม่มีกำหนดถูก scope เฉพาะ job type นี้จริง (มี contrast test ยืนยัน job type อื่นยัง `failed` ตามเดิม), LT-24 preflight มีสคริปต์+ผลจริงบันทึกใน `design.md`, เอกสาร role matrix/retry semantics ครบ, และ frontend `request()` handle empty 202 แล้วมี regression test ตรง. **แต่ P12-08 (test coverage) ยังไม่ปิดสนิท** — ปิดไปมากแล้ว (`LessonPurgeWorkerTests.cs`/`RevokedLinkPolicyTests.cs`/`CompanyIsolationTests.cs` ใหม่ครบทั้ง 60-day schedule, deferral, claim race, stale generation, daily retry, revoked-link policy ทุก case) แต่ยังขาด 3 จุดเจาะจง: (1) ไม่มี two-company test ของ `TryClaimPurge`/`TryRestoreAndCancelPurge`/`CancelPendingLessonPurge`/`AccelerateLessonPurge` ที่ระดับ repository/fake แม้แต่จุดเดียว (มีแค่ `GetTrash`/`GetIncludingDeleted` ที่ real-DB และ service-level cross-company ผ่าน `RestoreAsync`/`RequestPermanentDeleteAsync`), (2) ไม่มี test ของ shared-PDF-guard ตอน purge (สอง lesson อ้าง `PdfDocumentResourceId` เดียวกัน) และ idempotent-missing-external-delete ตามที่ task 530 ระบุ, (3) ไม่มี test ของ LT-8/QQ-1 ที่ trash ซ่อนคำถามแล้ว restore คืนคิว และไม่มี assertion ชัดเจนว่า `TrainingLink`/`LearningSession`/`BackgroundJob`/`LessonExcludedSlide` ถูกจัดการถูกต้องหลัง purge (แค่ question survival ถูกยืนยัน). ติ๊ก 13 checkbox เพิ่มใน `plan.md` (รวมเป็น 28/31 `[x]`); tasks 528/530/531 คง `[ ]`. Security-sensitive re-check เฉพาะ P12-03/P12-04 ยืนยันเป็น server-side ล้วน เลี่ยงไม่ได้จาก client. Automated checks ทั้งโปรเจกต์: frontend typecheck/lint clean, test **82/82** (+3), build clean (25 routes); backend Release build 0/0 (ฆ่า `SupportRoom.Api.exe` ค้าง PID ก่อน build), non-integration test **342/342** (+22 จาก test ใหม่), FULL suite 9 fail เหลือเฉพาะ external credential/network (Google/Gemini/Pinecone) ไม่ใช่ Module L, `has-pending-model-changes` clean. **Phase 12 ยังไม่ปิด 100% จึงยังไม่ deploy-eligible และยังบล็อก Phase 13 สอง task สุดท้ายตามเดิม (P12-08 เป็น failed re-check ครั้งที่ 1 ของรอบนี้ ยังไม่ถึงเพดาน 2 รอบ); 🔒 Security gate ยังเปิดตามเดิม, ไม่ได้ audit ในบทบาทนี้.** รายละเอียดเต็มอยู่ `_docs/module/knowledge-base/review.md`; Round 14 ย้ายไปเก็บใน `_docs/module/knowledge-base/review/phase-12.md` แล้ว. **Now: หยุดรอเจ้าของโปรเจกต์ตัดสินใจว่าจะส่ง P12-08 ที่เหลือ (3 sub-item เจาะจงข้างบน) กลับ `backend-engineer` อีกรอบ (แคบแล้ว ไม่ใช่งานใหม่) หรือยอมรับ risk ที่เหลือแล้วเดินหน้า `security` audit ก่อน — Phase 13 ยังห้าม dispatch จนกว่า Phase 12 จะปิด 31/31 จริง.**

</details>

<details>
<summary>ประวัติ Round 14 (FULL, Phase 12 first QA round — 15/31 verified, ก่อนถูกแทนที่ใน Round 15)</summary>

**⚠️ 2026-08-27 · `qa-engineer` Round 14 — FULL, Phase 12 (Module L) — first QA round, 15/31 verified, 4 Partial, 12 Failed:** ตรวจครบทุก task เทียบ R9, DM-2/DM-11/DM-15/DM-16/DM-18, MG-L1 และ LT-1..LT-24. ติ๊กเฉพาะ 15 task ที่ผ่านจริง; 16 task ที่ Partial/Failed คง `[ ]`. ไม่พบ Critical cross-company leak จาก `IgnoreQueryFilters()` และ raw SQL ที่ตรวจใช้ parameterized EF placeholders แต่พบ Important implementation gaps: archive race สร้าง purge job ซ้ำได้, restore ไม่เป็น transaction เดียวกับ job cancel, trash GET ไม่มี `cs` 403 ตาม task/LT-2, revoked token เรียก resume แบบ token-only แล้วยังได้ link/lesson metadata, `lesson_purge` กลายเป็น `failed` หลัง attempt 3 แทน retry 24 ชม.ไม่มีกำหนด, และ frontend parse body ว่างของ 202 จนรายงานล้มเหลวหลัง backend ตั้งคิวแล้ว. LT-24 preflight ไม่มีหลักฐาน; test matrices ของ tenant/public/worker/queue-purge ยังไม่ครบ. Automated checks: frontend typecheck/lint/test 79/79/build ผ่าน; backend Release build 0/0, non-integration test 320/320 และ EF pending-model clean; FULL backend suite 323 pass/11 fail จาก external credential/network integration. **Phase 12 ยังไม่ปิด/ไม่ deploy-eligible และยังบล็อก Phase 13 สอง task สุดท้าย; 🔒 Security gate ยังเปิดตามเดิม.** รายละเอียดอยู่ `_docs/module/knowledge-base/review.md`; Round 13 ย้ายไป `_docs/module/knowledge-base/review/phase-13.md` แล้ว. **Now: หยุดรอเจ้าของโปรเจกต์ตัดสินใจส่ง P12-01..P12-10 กลับ `backend-engineer`/`frontend-engineer`; ยังไม่ dispatch อัตโนมัติ.**

</details>

<details>
<summary>ประวัติ Round 13 (FULL, Phase 13 first QA round — 12/14 verified + 2 blocked, ก่อนถูกปิดใน Round 17)</summary>

**✅ 2026-08-27 `qa-engineer` Round 13 — FULL, Phase 13 (Module M) — first QA round, 12/14 verified ✅ (FULL), 2/14 correctly blocked (not failed):** ตรวจ NR-20..NR-24 (modal 3 สถานะ) และ CD-1..CD-10 (5 จุดแทนที่ `window.confirm`) ทั้งคู่เทียบ `design.md` ตัวจริง ไม่ใช่สรุปของ `plan.md`. ตรวจ three-part lock invariant ครบสามส่วนแยกกัน (NR-20: `commitModalOpen` เป็น state อิสระจริง ไม่คำนวณจาก `commitStarted`/`lessonId`/`committing` · NR-23: `onOpenChange={() => {}}` + `showCloseButton={false}` ชัดเจน ครบทุกสถานะ · NR-24: ตารางปุ่มครบ 6 กรณีรวม `failed` ทั้ง 4 ขั้นแยกกัน) — ทั้งสามผ่าน. ยืนยัน NR-21 ทั้งสอง call site (`commit()`/`handleRetryFailedNarrations()`) เลิกยิง `router.push` แล้ว ปลายทางใหม่ `/admin/lessons` มีจุดเดียว. ยืนยัน 4 จุดของ CD-5 ที่ทำแล้ว (จุด 1–4) ข้อความ/ปุ่ม/variant ตรงตาราง `design.md` คำต่อคำ ใช้ pattern `AlertDialog` inline เดิม ไม่ใช่ pattern พิมพ์ยืนยันของ `LessonPermanentDeleteDialog`; CD-9/CD-10 ตรวจแล้วไม่มีจุดใดถูกแตะผิด. จุดที่ 5 (`handleArchiveLesson`) และ task sweep สุดท้ายถูกปล่อยไว้ `[ ]` ถูกต้องตามกฎ R-35 ที่ห้ามแตะจนกว่า Phase 12 จะปิดรอบ QA — grep ทั้ง `frontend/src` แล้วเจอ `window.confirm` เหลือจุดเดียวตรงนั้นพอดี. บันทึก non-blocking observation หนึ่งข้อ: จุด 2/3 ของ CD-5 ไม่มี error handling เลยเมื่อ API ล้มเหลว (จุด 1/4 มี) — ตรวจสอบไม่ได้ว่าเป็นของเดิมหรือหลุดไปจากการรีแฟกเตอร์เพราะไม่มี git ให้ diff จึงบันทึกไว้ให้ยืนยันแทนที่จะฟันธงว่าเป็นบั๊ก. Automated checks: frontend typecheck/lint clean, test **79/79** (+10 จาก test ของ Module L/Phase 12 ที่มีอยู่แล้วในโค้ด ไม่เกี่ยวกับรอบนี้), build clean 24 routes; backend Release build ผ่านมี 8 warning เดิมในไฟล์ที่ไม่เกี่ยว (ไม่รัน backend test เพราะ phase นี้ไม่มีงาน backend เลย). ติ๊ก 12/14 checkbox ใน `plan.md`. **Phase 13 ยังไม่ deploy-eligible — ต้องรออีกหนึ่งรอบสั้นๆ (น่าจะ TARGETED) หลัง Phase 12 ปิดรอบ QA แล้ว `frontend-engineer` ทำจุดที่ 5 ให้เสร็จ.** ไม่ติด 🔒 Security gate (ยืนยันแล้ว ไม่มีเงื่อนไขใหม่เกิดขึ้น). ดูรายละเอียดเต็มที่ `_docs/module/knowledge-base/review.md` (current round).

</details>

**✅ 2026-08-26 `system-analyst` — amend รับ CR-5 / R10 เสร็จแล้ว · เกิด Module M (ไม่ติด 🔒 gate ·
frontend ล้วน · Data Model ไม่ถูกแตะ ไม่มี migration ไม่มี endpoint ใหม่):** NR-12 ท้ายข้อถูก
**กลับมติและแยกเป็นสองสัญญาที่ NR-21** (ต้องให้ CS กดยืนยันเอง · ปลายทาง `/admin/lessons`)
โดย **ลำดับ 4 ขั้นและข้อห้ามสลับ 3↔4 ไม่ถูกแตะ** · contract ใหม่ **NR-20..NR-24** +
**`## Confirmation Dialog Rules` CD-1..CD-10** (5 จุดของ `window.confirm` พร้อมข้อความเดิม
คำต่อคำ) · NR-13/NR-15 amend เฉพาะจุด · ความเสี่ยงใหม่ **R-32..R-36**
**✅ `Q-M1` ปิดแล้ว 2026-08-26** — เจ้าของโปรเจกต์เคาะ **ทางเลือก (ก) ล็อก modal ครบทุกสถานะ
ออกได้เฉพาะปุ่มในกล่อง** (Esc/คลิกนอกปิดไม่ได้เลย ทั้งตอนรัน สำเร็จ และล้มเหลว) · ตรงกับที่เขียนไว้
ใน NR-23/NR-24 อยู่แล้ว **จึงไม่มี contract ข้อใดถูกแก้ ไม่มี Data Model ไม่มี migration** ·
**ไม่มีข้อรอคำตอบค้างในโมดูลนี้แล้วแม้ข้อเดียว**
**Now**: `project-manager` วาง phase ของ **Module M** ได้ทั้งก้อน · **ข้อบังคับที่ต้องเขียนลง
หัวข้อ phase: ห้ามคร่อม Phase 12 (Module L)** · `qa-engineer` ต้องตรวจสามจุดจากมติ Q-M1 ให้ครบ
(ตารางสถานะ NR-24 · ห้ามผูก `open` กับ `commitStarted`/`lessonId` ตาม NR-20 ·
`showCloseButton={false}` ตาม NR-23)
**Blocked on**: ห้ามคร่อม Phase 12 (Module L) — โค้ด Module L มีอยู่จริงแล้วบางส่วนทั้งที่
`plan.md` ยัง `[ ]` ทุกช่อง (**R-35** · ตาม `conventions.md` §2 โค้ดชนะ index ให้ `qa-engineer`
ตรวจสถานะจริงของ Phase 12) · **R-34: อย่า deploy Phase 10 ก่อน Module M** — รอบนี้แก้โค้ดใน
เฟสที่ปิด FULL ไปแล้ว manifest ของ Round 8 จึงไม่ตรงอีกต่อไป

**✅ `qa-engineer` Round 12 — FULL, Phase 11 (Module K) ปิดจริง — 37/37 verified ✅ (FULL):**
รอบนี้คือการ re-check ครั้งที่ 3 ของ P11-01 บน architecture ที่เจ้าของโปรเจกต์เคาะเองตรงๆ
(เกินเพดาน 2 รอบของ pipeline แล้ว) — `LessonExcludedSlideReconciler.ReconcileAndLoad` (helper
ใหม่) จัดกลุ่มทุกแถวตาม `SlideObjectId` แล้ว hard-delete ทุกแถวส่วนเกินเหลือ representative เดียว
(เลือกแถว live ก่อน ถ้าไม่มีเลือกล่าสุด) เรียกใช้ร่วมกันทั้งสอง entry point (`ApplyExcludedSlidesAsync`
ฝั่ง SaveAsync และ `ToggleAsync` ฝั่ง EX-4) **ก่อน**ตรรกะอื่นใดของแต่ละ method และ commit ใน
transaction เดียวกับงานเขียนที่เหลือ — ตรวจจริงจากโค้ด ไม่เชื่อ approval ของเจ้าของโปรเจกต์เฉยๆ:
ไล่ scenario สองแถว live ซ้ำที่เคยพังสองรอบผ่าน `ToggleAsync` ตรงๆ, ยืนยันไม่มี commit ก่อนเวลา,
และยืนยันว่า regression test ยืนยัน end state ที่ถูกต้องจริง (เหลือแถวเดียว ไม่ live) ไม่ใช่แค่
"ไม่ throw" เหมือนสองรอบก่อน. ตรวจ 37/37 tasks จากศูนย์ทั้งเฟส ไม่ใช่แค่ P11-01. Automated checks
ทั้งโปรเจกต์: frontend typecheck/lint clean, test 69/69, build 19/19 routes; backend build 0/0
(Release), test 289/289 (+2 จาก Round 10), `has-pending-model-changes` clean; ไม่มี
`SupportRoom.Api.exe` ค้าง process ก่อน build. บันทึก observation เล็กน้อยไม่ blocking หนึ่งข้อ
(EX-8 hard-floor count นับซ้ำชั่วคราวได้ถ้ามี legacy duplicate ค้างที่หน้าอื่นของบทเรียนเดียวกัน —
self-heal, ทิศทางปลอดภัยเสมอ ไม่เคยปล่อยให้ต่ำกว่าพื้น). ติ๊ก checkbox EX-9 ตัวสุดท้ายใน `plan.md`
แล้ว — **Phase 11 ปิดจริง 37/37 [x], deploy-eligible ฝั่ง QA (FULL round) แล้ว เหลือแค่
`🔒 Security gate` ที่ยังไม่เคย audit เป็นตัวกันก่อนถึง `devops`.**
**Now: Phase 12 (Module L) เริ่มได้ตาม dependency Module K → Module L ที่ระบุใน
`plan.md`/`design.md` — ให้ `backend-engineer`/`frontend-engineer` หยิบงานที่ยังไม่เริ่มได้เลย
(ก่อนหน้านี้ Phase 12 เขียน task รอไว้แล้วแต่ห้ามมอบหมายจนกว่า Phase 11 จะปิด implementation);
`security` ควร audit Phase 11 (และ Phase 8/10 ที่ค้างมาก่อนหน้า) ก่อนส่ง `devops`.** ดูรายละเอียด
เต็มที่ `_docs/module/knowledge-base/review.md` (current round) และ
`_docs/module/knowledge-base/review/phase-11.md` (Round 9–12 ทั้งหมด รวม P11-01 สามรอบ).

<details>
<summary>ประวัติ Round 11 (TARGETED, P11-01 failed re-check 2 — ก่อนถูกปิดใน Round 12)</summary>

**⚠️ `qa-engineer` Round 11 — TARGETED, P11-01 failed re-check 2: Phase 11 ยัง 36/37 ·
ถึง escalation ceiling:** ตรวจ patch ทั้งสอง entry point และ regression tests ใหม่จากโค้ดจริง ·
ฝั่ง `SaveAsync` ปิดแล้ว: `ApplyExcludedSlidesAsync` cleanup duplicate ทุก group แม้ request ไม่แตะ
page นั้น และ hard-delete sibling จริง · แต่ direct EX-4 restore ยังผิด: `GetOne` แบบ ordered
`FirstOrDefault` ทำให้ไม่ throw แต่ `ToggleAsync(..., false)` soft-delete แค่แถวที่เลือกและปล่อย
legacy sibling live ค้างไว้ หน้าจึงยังถูกตัดหลัง endpoint ตอบสำเร็จ; test ใหม่ยืนยันสองแถวยังคงอยู่
และ `GetOne` คืน sibling ที่ live เอง · named tests ผ่าน 2/2 แต่ไม่ได้ขยาย FULL ตาม stop condition.
**Now: Phase 11 ยังไม่ปิด, Phase 12 ห้ามเริ่ม และ checkbox EX-9/P11-01 ยังไม่ติ๊ก · นี่คือ failed
re-check ครั้งที่ 2 จึงหยุดวนกลับ engineer อัตโนมัติและรอเจ้าของโปรเจกต์ตัดสินใจว่าจะอนุญาต backend
repair/re-check รอบถัดไปหรือไม่. Security gate ยังไม่ผ่านและยังไม่ได้เรียกในรอบนี้.** (ปิดแล้วใน
Round 12 ด้านบน — ดูสถานะปัจจุบันที่หัวหัวข้อนี้)

</details>

**✅ `frontend-engineer` Phase 11 / Module K implementation ตรวจครบแล้ว:** งาน `[frontend]`
ครบตาม EX-3/EX-4/EX-8/EX-9/EX-10/EX-11 และ NR amendments — type/domain + `api-client`
ตรง wire contract, แปลงเลขหน้าภาพจาก `slideObjectId`, หน้า narration แสดงทุกหน้าและ toggle
cut/restore ได้, draft exclusion ใน create phase ไม่เรียก endpoint ก่อนมี `LessonId`, UI กันตัดหน้า
สุดท้าย, warning PDF replacement นับ narration/exclusion แยกถูกต้อง. Automated checks: frontend
`typecheck`, `lint`, `test` (69/69), `build` ผ่าน. ไม่แตะ backend, contract docs หรือ checkbox.
**Now: Phase 11 รอ `qa-engineer` ตรวจ FULL หลังรวม backend handoff; ยังติด Security gate.**

**✅ 2026-08-26 (`qa-engineer`, Round 8 — FULL, Phase 8 + Phase 10, closing both):** Phase 8
(Module H, all 37 `plan.md` tasks) and Phase 10 (Module J, all 21 `plan.md` tasks) both
✅ Verified 100% by direct code inspection, zero ⚠️/❌ — the FULL round the deploy-eligibility
rule requires (Round 7 was TARGETED and could not close either phase on its own even at
100% checkbox-complete). Re-confirmed the P8-01/P8-02/NR-13 fixes are still genuinely in place,
re-ran every automated check project-wide (frontend typecheck/lint/test 69/69/build 24 routes;
backend build 0/0 Release, test 271/271, `has-pending-model-changes` clean), and re-read
`design.md`'s full Data Model — confirmed Phase 11/Module K's `DM-17`/`MG-K1` still has zero
code footprint (0/37 tasks), correctly out of scope. Restored `review.md`'s
`## Unverified Behaviour — undeployed phases` section, which had been dropped since Round 6.
**Both phases are now deploy-eligible on QA-mode grounds** — the only remaining blocker for
either is the standing `🔒 Security gate`, never audited. **Now: `security` should audit
Phase 8 and Phase 10 before either reaches `devops`; separately, Phase 11 (Module K) may now
start per R-26's condition (Phase 10 closed a FULL round) once `project-manager`/engineers pick
it up.** See `_docs/module/knowledge-base/review.md` for the full round.
...
**✅ `business-analyst` amend CR-5 / R10 เสร็จแล้ว · ไม่มีโค้ด ไม่แตะ checkbox
ไม่แตะ `design.md`/`plan.md`:** เพิ่ม **R10.1–R10.9** ลง `knowledge-base/requirement.md`
จากภาพหน้าจอที่เจ้าของโปรเจกต์ส่งมา + สัมภาษณ์ 7 คำถาม (CR-5.a–g) ·
**(ก) flow สร้างบทเรียน PDF**: ห้ามพาผู้ใช้ออกจากหน้าเอง ต้องกดยืนยันที่จอสรุปผล และปลายทาง
เปลี่ยนเป็น **หน้ารายการบทเรียนหลัก** · ทั้งช่วงรันและช่วงสรุปอยู่ใน modal ที่ **ปิดไม่ได้ระหว่างรัน**
(Esc/คลิกนอกไม่ได้) · ระหว่างรันแสดง **รายการขั้นตอนติ๊กทีละขั้น** · ล้มเหลวยังจบในกล่องเดิมโดย
retry รายขั้น + ลิงก์ทางออกของ NR-13 ต้องอยู่ครบ · คำเตือน NR-15 **แยกกล่องเหมือนเดิม** ·
**(ข)** เลิกใช้ `window.confirm` ครบ **5 จุด** (ลบหมวดย่อย · ลบเอกสาร · reset demo · ลบหมวด ·
ย้ายบทเรียนเข้าถัง) โดย **ไม่เปลี่ยนข้อความ/เงื่อนไข/สิทธิ์/ผลลัพธ์** ·
**⚠️ ไม่ใช่งานผิวหน้า: R10.1/R10.2 กลับมติ NR-12 ท้ายข้อ** (ที่ระบุว่าให้ `router.push`
ไปหน้าแก้บทเรียนทันทีเมื่อครบ 4 ขั้น) ทั้งเงื่อนไขและปลายทาง · **R10.6 ต้อง amend NR-13** ให้
สัญญาความล้มเหลวย้ายเข้ากล่องโดยข้อห้ามเดิมครบ ·
**ค้นแล้วทั้ง 5 จุดอยู่ในหน้าจอ `knowledge-base` ทั้งหมด** ไม่มีใน `company-admin`/`learning-session`
→ เป็น CR โฟลเดอร์เดียว ไม่ต้องแตกข้ามโมดูล · **อาการ "ไม่พบบทเรียนนี้ค่ะ" หลัง redirect
(slug ภาษาไทย) บันทึกเป็นเรื่องนอกขอบเขต** ให้ `system-analyst`/`qa-engineer` ตรวจแยก
**ห้ามถือว่า R10 คือการแก้อาการนี้** · **Now: พร้อมให้ `system-analyst` amend NR-12/NR-13
และตัดสิน mechanics ของกล่องยืนยัน; ⛔ ยังไม่ dispatch และยังไม่ส่ง engineer**

**✅ `project-manager` amend CR-4 / R9 เสร็จแล้ว · ไม่มีโค้ด ไม่แตะ checkbox:** เพิ่ม
**Phase 12 — Lesson trash, restore & permanent purge (R9) / Module L 🔒 Security gate** ใน
`knowledge-base/plan.md` หลัง Phase 11 · แตก task `[backend]`/`[frontend]` ครบ DM-2, DM-18,
MG-L1, LT-1..LT-24: trash/restore/manual queue, learnerKey-bound revoked-link policy, review queue,
durable `lesson_purge`, ordered external cleanup/shared-PDF guard, trash UI/countdown และ
tenant/race/idempotency tests · ห้ามเริ่มก่อน Phase 11 ปิด implementation; O-18 retention ต่อบริษัท
ยังไม่อยู่ใน task listตามคำสั่ง · **Now: Phase 12 พร้อมให้ `backend-engineer` และ
`frontend-engineer` หยิบตาม dependency เมื่อ Phase 11 ปิด implementation; `qa-engineer` ตรวจงานที่ทำเสร็จ
และ `security` ต้อง audit ก่อนส่ง devops**

**✅ `system-analyst` amend CR-4 / R9 เสร็จแล้ว · Data Model และ technical contract
ยืนยันแล้ว · ไม่มีโค้ด ไม่มี migration จริง ไม่แตะ checkbox ไม่แตะ `requirement.md`/`plan.md`:**
เพิ่ม **Module L — Lesson trash, restore & permanent purge 🔒 Security gate** หลัง Module K ·
เปิดใช้ soft-delete fields เดิมบน `LessonConfig` และเพิ่ม `PurgeJobId`/`PurgeStartedAt` · ออกแบบ
ตาราง `SessionQuestionReviewExclusion` สำหรับซ่อนคำถามถาวรหลัง purge · กำหนด additive migration
**MG-L1**, contract **LT-1..LT-24**, durable `lesson_purge` job, company-isolation rules และ
generation guard กัน stale job · ยืนยันมติ Q-L1..Q-L5: session เดิมเรียนจนจบแต่ห้ามเริ่มใหม่,
คำถามซ่อนขณะอยู่ในถังและกลับเมื่อ restore, shared primary PDF ลบเมื่อไม่มีบทเรียนอื่นอ้างถึง,
ครบ 60 วันแล้วยังมี `IN_PROGRESS` ให้รอและเช็กใหม่ทุกชั่วโมง, ไม่มีอีเมล · retention ต่อบริษัท
บันทึกเป็น **O-18 deferred และยังไม่ทำรอบนี้** จึงใช้ 60 วันตายตัว · **Now: พร้อมให้
`project-manager` เพิ่ม phase ของ Module L หลัง Module K; ยังไม่ได้ dispatch role ถัดไป**

**✅ `business-analyst` amend CR-4 / R9 เสร็จแล้ว · ไม่มีโค้ด ไม่แตะ checkbox
ไม่แตะ `design.md`/`plan.md`:** เพิ่ม requirement วงจรลบบทเรียนแบบ **เก็บเข้าถัง → กู้คืน →
ลบถาวร** · ถังเป็นแท็บในหน้าจัดการบทเรียนเดิมและทำทีละรายการ · `owner`/`admin` เก็บเข้าถัง
และกู้คืนได้ แต่ลบถาวรด้วยมือได้เฉพาะ `owner` พร้อมพิมพ์ชื่อบทเรียน · เข้าถังแล้วยกเลิกลิงก์เดิม
ทั้งหมด กู้คืนแล้วต้องสร้างลิงก์ใหม่ · auto-delete เมื่ออยู่ในถังครบ 60 วัน นับใหม่เมื่อกู้คืนแล้ว
เข้าถังซ้ำ · countdown แสดงเฉพาะในถัง (>14 ปกติ, 8–14 เหลือง, 1–7 แดง) ไม่มีอีเมลแจ้งเตือน ·
ลบถาวรแล้วลบ PDF บทพูด เอกสารและ Q&A scope บทเรียน รวมความรู้ที่ใช้ตอบ โดยไม่ย้าย scope ·
เก็บ session/คำถามเพื่อรายงานเป็น “บทเรียนที่ถูกลบ” แต่ไม่คืนคำถามสู่ active review queue ·
ยืนยัน company isolation · **Now: R9 พร้อมให้ `system-analyst` amend; ยังไม่ได้ dispatch role ถัดไป**

**✅ `business-analyst` ปิดช่องว่างสุดท้ายของ R9 (2026-08-26 · แก้ R9.1 ประโยคเดียว ไม่ใช่ CR ใหม่ ·
ไม่แตะ `design.md`/`plan.md`/checkbox):** R9.1 เดิมไม่ได้ระบุว่าจะทำอย่างไรกับผู้เรียนที่กำลังเรียน
ค้างอยู่ ณ เวลาที่เก็บบทเรียนเข้าถัง · เจ้าของโปรเจกต์เคาะว่า **ให้เรียนรอบที่ค้างจนจบ ปิดเฉพาะ
การเริ่ม/เข้าร่วมรอบถัดไป** · **ตรงกับมติ Q-L1 ที่ `system-analyst` ยืนยันไว้แล้ว จึงไม่กระทบ
Module L / Phase 12 และไม่ต้องตีกลับ** — เป็นการบันทึกกฎธุรกิจที่ขาดไปให้ตรงกับ design เดิม ·
**Now: ไม่มีคำถามธุรกิจค้างใน R9; stage ของโมดูลไม่เปลี่ยน**

**🆕 2026-08-26 (`business-analyst`, CR-3 · สัมภาษณ์ 13 คำถาม ถามทีละข้อ · ไม่มีโค้ด
ไม่แตะ checkbox ไม่แตะ `design.md`/`plan.md`):** เพิ่ม **R4.7 · ตัดหน้าออกจากบทเรียน PDF**
(R4.7.1–R4.7.12) ลง `requirement.md` · **ต้นเรื่อง:** เจ้าของโปรเจกต์รีวิวหน้าแก้บทพูดต่อหน้า
ที่ส่งมอบแล้ว แล้วถามเองว่าตัดหน้าทิ้งทั้งหน้าได้ไหม (PDF 10 หน้า ตัดหน้า 7 → ต้องทำตัวเหมือน
บทเรียน 9 หน้า ทั้งตอนสอนและตอนตอบ) โดยยกธง blast radius เอง ·
**⚠️ เป็นการกลับมติที่เพิ่งเคาะวันเดียวกัน** — CR-2.k ปฏิเสธ "จัดลำดับหรือซ่อนหน้า" ไปเมื่อ
2026-08-26 · `business-analyst` ถาม **CR-3.1 ก่อนคำถามอื่นทั้งหมด** เจ้าของโปรเจกต์ยืนยันว่า
กลับมติโดยตั้งใจ → แก้สองจุดที่เคยประกาศว่าตัดออกด้วยการขีดฆ่า + บันทึกเหตุผล ไม่ลบประวัติ ·
**กลับมติเฉพาะ "ซ่อนหน้า" · "จัดลำดับหน้า" ยังปิดอยู่ (ปฏิเสธซ้ำที่ CR-3.13)** ·
**มติ:** PDF เท่านั้น Google Slides ไม่แตะ · ตัดได้ทั้งในเฟสจัดการเนื้อหา (Module J) และ
หน้าแก้บทพูดของบทเรียนที่สร้างแล้ว · หน้าที่ตัด **ไม่ถูกสอนและใช้ตอบคำถามไม่ได้** โดย
**ถอด vector ออกจริงแบบ R6.1** และ **re-index อัตโนมัติแบบ R4 ข้อ 5** · เลขหน้าที่คนเห็น
เรียงใหม่ 1–9 (ไม่ได้เคาะเรื่อง id ภายใน — เป็นของ `system-analyst` ใต้ NR-4) ·
เอาหน้ากลับมาได้เสมอพร้อมบทพูดเดิม · **ต้องเหลืออย่างน้อย 1 หน้า (ข้อห้ามแข็ง)** ·
คนที่กำลังเรียนอยู่ใช้ชุดหน้าเดิมจนจบ มีผลรอบถัดไป · อัป PDF ทับใหม่ล้างรายการหน้าที่ตัดด้วย
และนับรวมในคำเตือน NR-3 · สิทธิ์ `cs`/`admin` เท่าเดิม ·
**CR-3.2 สำคัญกันตีความผิด:** เจ้าของโปรเจกต์ **ไม่ได้เลือก** เหตุผล "หน้าที่แปลงข้อความเสีย
(P4/R6.3)" แต่ตอบเองว่า *"เปิดโอกาสให้ปรับปรุง แก้ไข ตัดเนื้อหาที่ไม่จำเป็นได้"* →
**ห้าม scope ลงให้ใช้ได้เฉพาะเคสไฟล์เสีย** · ปฏิเสธชัด 4 ข้อ (จัดลำดับหน้า · รวม/แยกหน้า ·
แทรกหน้าเปล่า · โหมด "ข้ามตอนสอนแต่ยังใช้ตอบได้" ซึ่งขัด R4.7.3 ตรง ๆ) ·
**ผู้รับต่อ: `system-analyst` (amend)** — amend `NR-1..NR-19`, ตัดสินผลกับ `pdf-page-N`/chunk id
ใต้ **NR-4**, กลไกถอด vector, การไม่ให้การเรียนที่เปิดอยู่สะดุด, ลำดับงานกับ Module J
(TARGETED 21/21 แต่ยังไม่ FULL/ไม่ audit/ไม่ deploy) · ⛔ **ยังไม่ส่ง engineer**

**🆕 2026-08-26 (`system-analyst`, amend CR-3 · เจ้าของโปรเจกต์เคาะ Q-K1–Q-K4 ทีละข้อ ·
ไม่มีโค้ด ไม่แตะ checkbox ไม่แตะ `plan.md`/`requirement.md`):** `design.md` รับ R4.7 เข้าออกแบบ
ครบแล้ว → **Module K ใหม่ (🔒 Security gate)** · **⚠️ รอบนี้ Data Model ถูกแตะ ต่างจากรอบ CR-2**:
ตารางใหม่ **`DM-17 · LessonExcludedSlide`** + migration **`MG-K1`** (additive ล้วน · ไม่มีคอลัมน์ใหม่
บนตารางเดิม · ไม่มี backfill · `Down()` ย้อนกลับได้ 100%) · contract ใหม่
**`## Page Exclusion Rules (R4.7)` EX-1..EX-12** และ amend **NR-1/2/3/4/5/6/7/8/12/13/15/16/17/18**
· **ข้อสรุปสองข้อที่เป็นสาระ:** (1) **R4.7.6 ไม่ขัดกับ NR-4** — `SlideObjectId` กับ `Index` เป็น
คนละฟิลด์อยู่แล้ว การเรียงเลขใหม่ไม่แตะ id (นี่คือเหตุผลเดียวที่ R4.7.8 ทำงานได้)
(2) **หน้าหนึ่งมี vector สองใบ ไม่ใช่ใบเดียว** (`pdf-page-N` + `{documentId}-page-N` ตาม NR-8/CR-2.h)
→ **ถอดใบเดียว = R4.7.3(ข) ล้มทั้งข้อ** และเด็คเก่าที่ scope `company` (O-14) จะยังตอบได้ทั้งบริษัท ·
แก้ได้ด้วยเครื่องมือที่ R6.1 ทำไว้แล้วทั้งหมด ไม่มีกลไกใหม่ · **มติที่เคาะ:** Q-K1 = ตารางใหม่ ·
Q-K2 = รีเฟรชแล้วได้ชุดใหม่ทันที + ถอด vector ทันทีโดยยอมให้ตอบ `not_found` ระหว่างที่แท็บเก่า
ยังสอนอยู่ (**R-25** — `qa-engineer` ต้องไม่รายงานเป็น ❌) · Q-K3 = หน้าแก้บทพูดแสดงครบทุกหน้า
การ์ดสีจาง + read-only + ปุ่มเอากลับ · Q-K4 = **แยกเป็น Module K และเริ่มหลัง Module J ปิดรอบ
FULL เท่านั้น (R-26)** · **ผู้รับต่อ: `project-manager`** (วาง phase ของ Module K โดยเคารพ R-26) ·
⛔ **ยังไม่ส่ง engineer**
...
- **⚠️ แก้ความเข้าใจที่ส่งต่อกันมาผิด 2 จุด (ยืนยันด้วยโค้ดจริง)**: (1) **R7.6 ยังไม่เสร็จทั้งข้อ** —
  ...
  `frontend/src/components` ไม่พบ call site แม้แต่จุดเดียว** → **ฝั่ง UI ยังเป็นงานเต็มใบ**
  (เอกสารรับรองส่วนที่ทำไปแล้วย้อนหลัง + บันทึก R-19 ว่าของชุดนี้ไม่เคยผ่าน QA และไม่มี checkbox
  ไหนครอบ) (2) **ที่อยู่โค้ดที่ R8 ต้องแตะเปลี่ยนไปแล้ว** — การ์ด "เอกสารประกอบ" อยู่ที่
...
- **✅ 2026-08-26 (`project-manager`): Phase 10 (Module J) อยู่ใน `plan.md` แล้วครบ** — ตรวจแล้วว่า
  Phase 10 ที่มีอยู่ก่อนหน้าครอบ NR-10..NR-19 ตาม `design.md` §Module J ทุกข้อ ไม่มีช่องว่างต้องเพิ่ม
  task · **แก้เฉพาะถ้อยคำ R-24 sequencing note**: ตรวจโค้ดจริง (`LessonForm.tsx`) แล้วพบว่า Phase 9
  (Module I) **merge เป็นโค้ดจริงแล้ว** (การ์ด `DocumentUploadList`/`fixedScope` เดิมถูกแทนที่ด้วย
  บรรทัดสรุปอ่านอย่างเดียว, ตรงกับ commit `91e6d19` "Module H/I ครบ") — เดิม `plan.md` เขียนว่า
  "Phase 9 ยังไม่มีโค้ดเขียนจริงแม้บรรทัดเดียว" ซึ่งไม่ตรงของจริงแล้ว แก้ให้ตรง แต่**บทสรุปเชิง
  ปฏิบัติไม่เปลี่ยน** — checkbox ของ Phase 9 ยังไม่มีช่องใดถูก `qa-engineer` ติ๊ก จึงยังห้าม dispatch
  งาน `[frontend]` ของ Phase 10 จนกว่า Phase 9 จะผ่าน QA จริง (`[backend]` ของ Phase 10 ไม่ติดเงื่อนไข
  นี้ ทำก่อนได้) · ไม่แตะ checkbox ใดทั้ง Phase 9/10 · **ขั้นถัดไป: `frontend-engineer`/
  `backend-engineer` หยิบงาน `[backend]` ของ Phase 10 ได้ทันที ส่วน `[frontend]` รอ Phase 9 ผ่าน
  `qa-engineer` ก่อน** · แยกเรื่อง: 🟡 pacing/Module P ที่ `company-admin` (ดูหัวข้อ
  "🟡 เรื่องที่ค้างการตัดสินใจ" ด้านล่าง) **ยังบล็อกอยู่ที่ `system-analyst` ไม่ใช่ PM** — ไม่ได้แตะ

- **⚠️ `qa-engineer` FULL Phase 8 (Module H): ลงผลแล้วและยังไม่ dispatch ตามคำสั่งผู้ใช้** — ตรวจ
  checkbox จริงครบ 37 ข้อ เทียบ R7, KL-1..KL-26, DM-3/DM-15/MG-H1, โค้ด, migration และ tests;
  ติ๊กผ่าน **36/37** เหลือ task layout/filter ที่ `/admin/documents` ยัง `[ ]` เพราะ R7.3 ขอ filter
  ตาม `IndexingStatus` แต่ shared toolbar มีเพียง scope + search และ effect ของทั้งตารางเอกสาร/Q&A
  ไม่ฟัง `filter.status` (**P8-01 Important**) · ไม่พบ `IgnoreQueryFilters()` ใน active list/search
  path และ KL-11 ใช้ `EF.Functions.ILike` ผ่าน captured parameter ทั้งสอง service โดยไม่มี raw SQL;
  อย่างไรก็ดี test ตาม R-16 ที่ seed **สองบริษัท** แล้วพิสูจน์ list เอกสารและ Q&A ผ่าน
  `GetAllInCompany()` จริงยังขาด (**P8-02 Important**) · frontend lint/typecheck/test 69/69/build
  ผ่าน; backend Release build ผ่าน, test 269/269 ผ่าน, EF ไม่มี pending model changes · Phase 8 =
  **⚠️ Partial, 0 Critical, ยังไม่ผ่าน Security gate และยังไม่ส่งต่อ role ใด** · รายละเอียดและ
  manifest อยู่ที่ `_docs/module/knowledge-base/review.md`; Round 5 ของ Phase 10 เก็บไว้ที่
  `_docs/module/knowledge-base/review/phase-10.md`
...
โมดูล (`conventions.md` §1) ต้องมีคนตัดสินว่าใครแก้ · **`project-manager` ก็ไม่แก้ด้วยเหตุผลเดียวกัน
(ไฟล์อยู่นอก `_docs/module/`) — ยังต้องมีคนตัดสินใจว่าใครแก้**
