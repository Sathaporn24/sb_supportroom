...
> **Data Model ไม่เปลี่ยนแม้แต่ฟิลด์เดียวในรอบนี้ และไม่มี migration ใหม่** — ยกเว้น R7.5
> ที่ค้างรอมติ **Q-H1** อยู่ (ดู `## Unresolved Open Questions`) · DS-8/Q-C (โหมด `fixedScope`)
> **ถูกยกเลิกตามมติ CR-1.j** ดู DS-8 ที่แก้แล้ว
...
| **R7.5** | เตือนเมื่อชื่อหรือเนื้อหาซ้ำ | **⏸️ ชื่อไฟล์ = ทำได้ทันที · เนื้อหา = ต้องมีคอลัมน์ใหม่ รอมติ Q-H1** | **นี่คือจุดเดียวของ R7/R8 ทั้งชุดที่แตะ schema** · "ชื่อซ้ำ" เทียบ `FileName` ได้เลยไม่ต้องเพิ่มอะไร · "เนื้อหาซ้ำ" ต้องมีลายนิ้วมือของไฟล์ (`ContentHash`) เพราะไม่มีอะไรในตารางวันนี้เทียบเนื้อหาได้ (`SizeBytes` เท่ากันไม่ได้แปลว่าไฟล์เดียวกัน) → **ค้างที่ Q-H1 ห้าม implement ก่อนเคาะ** · Q&A คำถามซ้ำเป๊ะเตือนได้โดยไม่ต้องมีฟิลด์ใหม่ |
...
ไม่มี service ภายนอกใหม่ และ — ยกเว้น Q-H1 ที่ยังไม่เคาะ — ไม่มี migration ใหม่**
...
| **KL-17** | **ขอบเขตของ R7.6 ที่ห้ามขยาย** — แก้/ลบ **ทีละรายการเท่านั้น** · **ไม่มี** bulk · **ไม่มี** audit trail ของการแก้แต่ละครั้ง (R5.6 ขอแค่รู้ว่าใครเขียน/เมื่อไหร่ ซึ่ง `CreateBy`/`CreateDate`/`UpdateBy`/`UpdateDate` มีอยู่แล้ว) · **ไม่มี** การกู้คืน Q&A ที่ลบไปแล้ว (soft delete มีจริงแต่ไม่มี UI — ต่างจากเอกสารที่มี `DeletedDocumentsList` เพราะ R6.1 ขอไว้ ส่วน Q&A ไม่มีใครขอ) · สิทธิ์แก้/ลบของคนอื่นในบริษัทเดียวกันเป็นไปตาม **QQ-9/O-1 เดิม ไม่เปลี่ยน** |
| **KL-18** | **⏸️ R7.5 (เตือนซ้ำ) ยังไม่เป็น contract — ค้างที่ Q-H1** · ห้าม implement ข้อนี้จนกว่าเจ้าของโปรเจกต์จะเคาะ · สิ่งที่ **แน่นอนแล้วไม่ว่ามติจะออกทางไหน**: (1) เป็นการ **เตือน ไม่ใช่บล็อก** — CS ยืนยันอัปโหลดต่อได้เสมอ (2) เทียบเฉพาะภายในบริษัทเดียวกัน และไม่นับแถวที่ถูก soft delete ไปแล้ว (3) เตือนตอน **กำลังจะอัปโหลด/กำลังจะบันทึก Q&A** ไม่ใช่มาไล่หาของซ้ำย้อนหลังในคลัง (4) Q&A ที่ `Question` ซ้ำเป๊ะหลัง trim+ยุบช่องว่าง ให้เตือนได้เลย **ไม่ต้องรอ Q-H1** เพราะไม่ต้องใช้ฟิลด์ใหม่ · สิ่งที่ **ค้างจริงคือ "เนื้อหาไฟล์ซ้ำ"** ซึ่งต้องมีคอลัมน์ `ContentHash` |
...
| **UC-9** | **หน้า `/admin/lessons/new` ไม่มีอะไรต้องแก้จากข้อนี้** — ตรวจแล้ว 2026-08-25: การ์ดเอกสารประกอบถูก render เฉพาะเมื่อมี `lesson` (แก้บทเรียนที่มีอยู่) หน้าสร้างใหม่จึงไม่เคยมีมันอยู่แล้ว · UC-6 ก็ไม่ต้องโผล่ที่หน้าสร้างใหม่ด้วยเหตุผลเดียวกัน (ยังไม่มี `lesson.id` ให้กรอง) |
...
**Module H (R7 / KL-1..KL-18) และ Module I (R8 / UC-1..UC-10) ไม่มี migration เช่นกัน**
(เพิ่ม 2026-08-25) — ทั้งสองโมดูลเป็นทางอ่านและหน้าจอล้วน: `GetAllInCompany()` ทั้งสองตัวใช้ตาราง
และ index ที่มีอยู่แล้ว · การค้นเนื้อหา (KL-11) อ่าน `DocumentChunk.Text` ที่ MG-D1 สร้างไปแล้ว ·
Q&A ทั้งชุดมาจาก MG-F1 · **ข้อยกเว้นเดียวคือ R7.5 ซึ่งยังไม่ใช่ contract** — ถ้ามติ **Q-H1**
ออกมาเป็นทาง B จะเกิด migration ใหม่หนึ่งใบ (`AddDocumentContentHash`: `ContentHash` เป็น
`string?` + index `(CompanyId, ContentHash)` · **additive ล้วน ไม่ backfill ไม่แตะข้อมูลเดิม**)
ซึ่งต้องกลับมา amend หัวข้อนี้และ `## Data Model` ก่อน **ห้าม generate ล่วงหน้า**
...
(KL-14..KL-17) · **ไม่มี migration ไม่มีฟิลด์ใหม่**
...
และ Phase 7 verified ✅ (FULL)** จึงเริ่มได้ทันที · **⛔ R7.5 (KL-18) ห้ามหยิบไปทำจนกว่า Q-H1 จะเคาะ**
...
> ⛔ **มีคำถามค้าง 1 ข้อ (Q-H1) ที่บล็อกเฉพาะ R7.5 — ที่เหลือของ R7/R8 ไม่ถูกบล็อก
> และ Module A–G ไม่ถูกกระทบเลย**

### ⏸️ ค้างอยู่จริง — รอเจ้าของโปรเจกต์เคาะ (2026-08-25)

| # | คำถาม | ทางเลือก | บล็อกอะไร |
|---|---|---|---|
| **Q-H1** | **R7.5 "เตือนเมื่อเนื้อหาซ้ำ" จะรู้ได้อย่างไรว่าเนื้อหาซ้ำ?** — ไม่มีอะไรในตารางวันนี้เทียบเนื้อหาไฟล์ได้ (`SizeBytes` เท่ากันไม่ได้แปลว่าไฟล์เดียวกัน · `DocumentChunk.Text` มีก็ต่อเมื่อ index สำเร็จไปแล้ว = สายเกินกว่าจะเตือนตอนอัปโหลด และจ่ายค่า embedding ไปแล้ว) | **A · เตือนเฉพาะชื่อไฟล์ซ้ำ** — เทียบ `FileName` (trim + ไม่สนตัวพิมพ์) ในบริษัทเดียวกัน · **ไม่มี schema change เลย** ทำได้ทันที · **ราคา: ทำ R7.5 ได้ไม่ครบตามที่ CR-1.h เคาะไว้** ไฟล์เดิมที่ถูกเปลี่ยนชื่อจะไม่ถูกจับ ซึ่งเป็นเคสที่เกิดจริงบ่อยที่สุด<br>**B (ข้อเสนอของ `system-analyst`) · ชื่อไฟล์ + ลายนิ้วมือไฟล์** — เพิ่ม `DocumentResource.ContentHash` (SHA-256 hex 64 ตัว, `string?`, คำนวณจาก byte stream ตอนอัปโหลดก่อนส่งเข้า storage) + index `(CompanyId, ContentHash)` · **additive ล้วน ไม่แตะข้อมูลเดิมแม้แถวเดียว** · **ราคาที่ต้องยอมรับ: เอกสารที่อัปไปแล้วก่อนหน้านี้จะไม่มีค่า hash และจะไม่ถูกจับว่าซ้ำตลอดไป** (การ backfill ต้องดาวน์โหลดไฟล์เก่าทุกใบจาก object storage มาคำนวณใหม่ — ไม่คุ้ม ไม่แนะนำ) | **บล็อกเฉพาะ R7.5 / KL-18** · **ไม่บล็อก** R7.1/R7.2/R7.3/R7.4/R7.6 และไม่บล็อก R8 ทั้งชุด · `project-manager` วาง phase ของ Module H/I ได้เลยโดยกันงาน R7.5 ออกไว้เป็นก้อนแยก |

**สิ่งที่เคาะไว้แล้วไม่ว่า Q-H1 จะออกทางไหน** อยู่ที่ **KL-18** (เตือนไม่บล็อก · ภายในบริษัทเดียวกัน ·
ไม่นับของที่ลบไปแล้ว · เตือนตอนกำลังจะบันทึก ไม่ใช่ไล่หาย้อนหลัง · **Q&A ที่คำถามซ้ำเป๊ะเตือนได้เลย
ไม่ต้องรอมตินี้**)
...
| **O-12** | **"ซ้ำ" ตาม R7.5 คือซ้ำแบบเป๊ะเท่านั้น ไม่ใช่ซ้ำแบบใกล้เคียง** (เพิ่ม 2026-08-25) | ทั้งทาง A และ B ของ Q-H1 จับได้แค่ "ไฟล์เดียวกัน/ชื่อเดียวกัน/คำถามเดียวกันเป๊ะ" · เอกสารคนละไฟล์ที่เนื้อหาทับกัน 90% หรือ Q&A ที่ถามเรื่องเดียวกันด้วยคำต่างกัน **จับไม่ได้และไม่พยายามจับในเฟสนี้** — การจับแบบนั้นต้องใช้ embedding เทียบความคล้าย ซึ่งเป็นคนละเรื่องกับ R7 (การมองเห็น) และแตะ retrieval ที่ CR-1.g สั่งห้าม · ถ้าวันหนึ่งต้องการจริง **ต้อง amend ก่อน** |
