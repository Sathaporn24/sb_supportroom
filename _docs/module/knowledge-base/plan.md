...
- [x] [backend] unit/integration tests — tenant isolation LT-23: company A ใช้ lesson/link/job id ของ B กับ every trash/restore/manual-purge/purge-repository path ได้ not found/forbidden และข้อมูล B ไม่เปลี่ยน; cover every `IgnoreQueryFilters()` path explicitly
